---
title: "⚡ Building Transformers from Scratch with PyTorch · Day 4 — Scaled Dot-Product Attention from Scratch"
page-layout: full
toc: true
toc-location: left
...
Yesterday you gave every token an identity: a token embedding that says *what* it is, plus a positional embedding that says *where* it sits. But each token's vector is still a hermit — it knows nothing about its neighbours. `"bank"` in *river bank* and *bank account* comes out of Day 3's pipeline as the exact same vector. Today we fix that. We build **scaled dot-product attention**, the mechanism that lets every token look back over the sequence, decide which earlier tokens matter to it, and pull in a weighted blend of their information. This is the single most important component in the entire course — everything from Day 5 onward is just packaging around what you build today. (For the theory-first treatment, see the *Attention & Transformers* chapter in the encyclopedia; here we build it line by line and verify it by hand.)

::: {.callout-note appearance="simple"}
🎯 **Today you will:** project embeddings into queries, keys and values with `nn.Linear`, compute attention scores as $QK^\top/\sqrt{d_k}$, build the causal mask with `tril` + `masked_fill`, wrap it all into a reusable single-head module with dropout, and verify the whole thing against a hand-computed example
:::
...
Set up a tiny, fully reproducible playground. We keep the `(B, T, C)` naming convention from the earlier days: **B**atch, **T**ime (sequence length), **C**hannels (embedding dimension).
...
x = torch.randn(B, T, C)   # stand-in for Day 3's output: token emb + pos emb
```

`x` is exactly what your Day 3 pipeline produces: a batch of sequences where each token is already a `C`-dimensional vector carrying identity + position. Now the three projections:
...
|---|---|---|
| `x` | `(B, T, C)` | embedded input from Day 3 |
...
Notice row 0: the first token has no past, so 100% of its attention lands on itself. Every later row spreads its unit budget over strictly more options. When you train the real model on Day 8 and plot these matrices, you'll see structure emerge — heads that lock onto the previous token, heads that find matching brackets, heads that track the subject of a sentence.
...
Now we wrap the whole computation in an `nn.Module` you'll reuse verbatim inside Day 5's multi-head attention. Two production details get added: **dropout on the attention weights** (randomly zeroing some connections so the model can't over-rely on any single token-to-token edge), and the mask stored as a **buffer**.
...
- **`self.tril[:T, :T]`** — we allocate the mask once at the maximum context length (`block_size`, from Day 2) and *slice* it to the actual sequence length at runtime. Sequences shorter than `block_size` (common in generation, where the prompt starts tiny) just use the top-left corner. Rebuilding the mask every forward pass works too, but allocates a fresh tensor on every call for no reason.
...
Tomorrow: one head is one "conversation channel" — Day 5 runs many heads in parallel and concatenates them, so the model can track syntax, coreference, and position all at once.
