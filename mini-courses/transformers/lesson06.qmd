---
aliases: ["/mini-courses/transformers/day06.html"]
title: "⚡ Building Transformers from Scratch with PyTorch · Lesson 6 — The Transformer Block: Where Everything Clicks Together"
page-layout: full
toc: true
toc-location: left
...
In the previous lesson you built multi-head attention — the machinery that lets every token gather information from every earlier token. But attention alone is not a transformer. If you stack raw attention layers on top of each other, gradients die, activations drift, and the network refuses to train past a few layers. In this lesson you build the **transformer block**: the repeating unit that wraps attention and a feed-forward network in residual connections and layer normalization, arranged so precisely that you can stack it 12, 48, or 96 times and it *still trains*. By the end of this lesson you'll have a `Block` class and a first two-block model pushing real tensors end to end — the exact structure GPT-2 uses, just smaller.
...
flowchart TB
    ...
    LN1 --> MHA["Multi-Head Attention<br/>(Lesson 5)"]
    MHA --> ADD1(("+"))
    X --> ADD1
...
This picture also explains a hard constraint you already met on Lesson 3: the embedding dimension `n_embd` is the width of this belt, and *everything* — token embeddings, positional embeddings, every block's output — must speak in exactly this width, because they're all added into the same stream.
...
One consequence to remember for in the next lesson: because pre-norm never normalizes the stream itself, the *final* output of the last block can have grown large — so the full GPT model adds one extra `LayerNorm` after the last block, before the output head. We'll place it on Lesson 7.

## Assembling the Block

We have all three ingredients. Bring in in the previous lesson's attention (shown here compactly so this lesson's file runs standalone — if you have `day5.py`, just `from day5 import MultiHeadAttention`):

```python
import torch.nn.functional as F

class MultiHeadAttention(nn.Module):
    """Lesson 5's module, condensed: fused QKV, causal mask, output projection."""
...
And now the star of the lesson. Read it slowly — this is arguably the most important dozen lines of the whole course:
...
The payoff of shape-preservation: stacking blocks is a `for` loop. Here's a minimal harness — embeddings from Lesson 3 feeding two blocks (the output head and loss arrive on Lesson 7):

```python
class TinyStack(nn.Module):
    """Embeddings -> N transformer blocks. The torso of Lesson 7's GPT."""
...
model = TinyStack(vocab_size=65)                 # 65 = Lesson 2's tiny-shakespeare charset
...
import torch
import torch.nn as nn
# assumes FeedForward and MultiHeadAttention from this lesson's lesson are defined
...
In the next lesson we crown the stack: a final LayerNorm, the language-model head, weight tying, and proper initialization — the full GPT class, ready to compute a loss.
