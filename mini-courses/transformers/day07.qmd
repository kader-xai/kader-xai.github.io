---
title: "⚡ Building Transformers from Scratch with PyTorch · Day 7 — Assembling the Full GPT"
page-layout: full
toc: true
toc-location: left
...
```{mermaid}
flowchart TB
    A["idx : (B, T) token IDs"] --> B["tok_emb — Day 3\n(B, T, C)"]
    P["pos_emb — Day 3\n(T, C)"] --> C
    B --> C["add + dropout\n(B, T, C)"]
    C --> D
    ...
    G --> H["ln_f : final LayerNorm\n(B, T, C)"]
    H --> I["lm_head : Linear C → vocab\n(B, T, V)"]
    I --> J["logits"]
    J --> K["cross_entropy vs targets\nscalar loss"]
    T2["targets : (B, T)"] --> K
```
