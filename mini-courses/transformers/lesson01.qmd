---
aliases: ["/mini-courses/transformers/day01.html"]
title: "⚡ Building Transformers from Scratch with PyTorch · Lesson 1 — The Transformer Map: What We're Building"
page-layout: full
toc: true
toc-location: left
...
1. **Input**: a batch of token-ID sequences, shape `(B, T)` — `B` sequences, each `T` integers. Just numbers like `[31, 4, 56, ...]`. Lesson 2 builds the pipeline that produces these.
2. **Embeddings**: each ID becomes a learned vector of width `n_embd`, and each *position* 0..T-1 contributes its own learned vector. Add them: `(B, T, n_embd)`. Lesson 3.
3. **Blocks**: `n_layer` identical transformer blocks, each refining the representation without changing its shape — `(B, T, n_embd)` in, `(B, T, n_embd)` out. This shape-preservation is what lets you stack as many as you can afford. Lessons 4–6.
...
5. **Loss or sample**: during training, compare logits at position $t$ against the true token at $t+1$ via cross-entropy (Lesson 8). During generation, softmax the last position's logits and sample (Lesson 9).
