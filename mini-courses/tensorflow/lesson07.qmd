---
aliases: ["/mini-courses/tensorflow/day07.html"]
title: "📊 Deep Learning with TensorFlow & Keras · Lesson 7 — Regularization & Tuning: Making the CNN Behave"
page-layout: full
toc: true
toc-location: left
...
# Lesson 7 — Regularization & Tuning: Making the CNN Behave

In the previous lesson's CNN learned CIFAR-10 — and then kept learning it a little too well. If you let the Lesson-6 model run for 30 epochs, you saw the classic signature: training accuracy marching toward 99% while validation accuracy stalled in the low 70s and then drifted *down*. The network stopped learning the dataset and started memorizing it. In this lesson we fix that, and we stop guessing at hyperparameters while we're at it. We'll upgrade the Lesson-6 CNN with the four workhorses of practical deep learning — BatchNorm, Dropout, L2 weight decay, and a cosine learning-rate schedule — turn on mixed precision with a single line, and then let `keras_tuner` search the knobs we'd otherwise set by superstition. At the end we put the upgraded model head-to-head against the baseline.

::: {.callout-note appearance="simple"}
🎯 **In this lesson you will:** place Dropout and BatchNorm correctly inside a conv block, add L2 weight decay via `kernel_regularizer`, schedule the learning rate with warmup + `CosineDecay`, enable mixed precision in one line, and run a `keras_tuner` RandomSearch over units and learning rate
:::

## Baseline: where Lesson 6 left us

Let's reconstruct a compact version of the Lesson-6 setup so this lesson is self-contained. Same data, same `tf.data` pipeline from Lesson 3, same plain CNN.
...
Nothing new here — `(45000, 32, 32, 3)` flows in, batches of `(128, 32, 32, 3)` flow out. The baseline model is Lesson 6's architecture, unregularized:
...
- **`fit()` adds `model.losses` for you; a custom training loop does not.** If you write your own `GradientTape` loop (Lesson 4) and forget `loss += tf.add_n(model.losses)`, your regularizer silently does nothing. No error, no warning — just quietly absent regularization. This is one of the most common "why doesn't L2 change anything?" bugs.
...
|---|---|---|---|---|---|
| Lesson-6 baseline | 98.7% | 71.9% | 71.2% | 26.8 pts | 11 s |
...
In the next lesson: why train convolutional features from scratch at all — we bolt a pretrained ImageNet backbone onto our pipeline with Keras Applications and get better accuracy in a fraction of the epochs.
