---
aliases: ["/mini-courses/tensorflow/day02.html"]
title: "📊 Deep Learning with TensorFlow & Keras · Lesson 2 — Keras Models Three Ways: Sequential, Functional, Subclassing"
page-layout: full
toc: true
toc-location: left
...
Under the hood, that first call ran `dense.build(input_shape=(32, 784))`, which created two `Variable`s — exactly the kind you managed by hand in the previous lesson, except now the layer owns them, tracks them, and will hand them to an optimizer for you. The computation is the one you already know:
...
2. **The model is inspectable and sliceable.** You can make a new model out of any subgraph — `keras.Model(inputs, fn_model.layers[1].output)` gives you a feature extractor with zero copying. Lesson 8's transfer learning leans on this constantly.
...
If you wanted two *independent* towers, you'd instantiate two `Dense` layers. Instance identity is the sharing mechanism — there is no special "share" flag. (This lesson's exercise builds on exactly this.)
...
`get_weights()`/`set_weights()` works here because the three builds create the same variables in the same order. Architecture and weights are separable — an idea that pays off again on Lesson 9 when we save and reload models.
...
Every layer and model exposes its variables three ways, and the distinction matters as soon as you freeze layers (Lesson 8) or add BatchNorm (Lesson 6):
...
In the previous lesson you passed a hand-curated variable list to `tape.gradient(...)`. From now on, `model.trainable_weights` *is* that list, maintained for you — that's the whole handoff between Lesson 1 and Lesson 4's custom training loops.
...
- `model.weights` / `trainable_weights` / `non_trainable_weights` is the handoff from Lesson 1's hand-managed variables; `trainable = False` moves variables between the last two.
...
In the next lesson: your models stop eating NumPy arrays from memory and start drinking from a firehose — `tf.data` pipelines that shuffle, batch, prefetch, and keep the GPU fed.
