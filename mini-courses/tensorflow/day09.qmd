---
title: "📊 Deep Learning with TensorFlow & Keras · Day 9 — Saving & Deployment: From `.keras` to the Edge"
page-layout: full
toc: true
toc-location: left
...
# Day 9 — Saving & Deployment: From `.keras` to the Edge

You've spent eight days building models — from raw tensors and `GradientTape` all the way to a fine-tuned transfer-learning classifier. Every one of those models died when the Python process exited. Today we fix that. A trained model is worth nothing until it answers requests from something that isn't your notebook: a web backend, a Docker container fielding REST calls, a phone, a browser. TensorFlow's deployment story is arguably its strongest card versus PyTorch, and today you'll play the whole hand: the `.keras` format for checkpointing, `model.export()` for serving, TensorFlow Serving in Docker, and TFLite quantization to shrink your model 4× for edge devices. We'll close with the question people ask most — *"so… PyTorch or TensorFlow?"* — answered honestly.

::: {.callout-note appearance="simple"}
🎯 **Today you will:** save and reload a model with the `.keras` format, export a SavedModel with a custom serving endpoint, serve it via TensorFlow Serving in Docker and call it over REST, convert and int8-quantize it with TFLite, and learn a decision framework for choosing between TensorFlow and PyTorch
:::

## A model to deploy

We need something trained to work with. To keep today self-contained, here's a small Fashion-MNIST classifier — three epochs, under a minute on CPU. (If your Day 8 fine-tuned model is still around, everything below works on it identically; the artifacts are just bigger.)
...
| Consumed by | You, tomorrow | TF Serving, TFLite, TF.js, C++/Java/Go runtimes |
...
flowchart LR
    ...
    M -- "model.save()" --> K[".keras file\ncheckpoint / resume"]
    M -- "model.export()" --> S["SavedModel\nserving/fashion/1"]
    K -- "load_model()" --> M
    S --> TFS["TF Serving (Docker)\nREST :8501 / gRPC :8500"]
    S -- "TFLiteConverter" --> TFL[".tflite\nint8, 4x smaller"]
    S -- "tensorflowjs_converter" --> TJS["TF.js\nbrowser / Node"]
...
That's the course: nine days from a bare tensor to a quantized model answering HTTP requests — from here, the MLOps mini-course takes your container the rest of the way to production.
