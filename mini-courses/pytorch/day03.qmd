---
title: "🔥 Deep Learning with PyTorch · Day 3 — Feeding the Model: Dataset & DataLoader"
page-layout: full
toc: true
toc-location: left
...
# Day 3 — Feeding the Model: Dataset & DataLoader

Yesterday you built an `nn.Module` and called it on tensors you created by hand. That works for a demo, but real training data doesn't arrive as one convenient tensor sitting in memory — it lives in CSV files, image folders, databases, and it's usually too big, too messy, or too slow to load all at once. Today you build the pipe that connects raw data on disk to the `forward()` you wrote yesterday: PyTorch's `Dataset` and `DataLoader`. By the end you'll have a complete tabular pipeline — CSV file → custom `Dataset` → train/val split → shuffled, batched, GPU-ready mini-batches — which is exactly what Day 4's training loop will consume.

::: {.callout-note appearance="simple"}
🎯 **Today you will:** write a custom `Dataset` with `__len__` and `__getitem__`, apply transforms without leaking validation statistics, split data with `random_split`, configure a `DataLoader` (batching, shuffling, `num_workers`, `pin_memory`), and assemble an end-to-end tabular pipeline that feeds yesterday's model.
:::
...
Keep this picture in mind: everything in today's lesson is one box in that diagram.
...
2. **It returns *one* sample, not a batch.** The `DataLoader` stacks samples for you. If you return shape `(1, 8)` "to be safe", your batches come out `(batch, 1, 8)` instead of `(batch, 8)` and yesterday's `nn.Linear(8, ...)` will either crash or — worse — broadcast in a way that trains garbage.
...
We need a validation set the model never trains on, so we can detect overfitting on Day 4. `random_split` partitions a dataset into random, non-overlapping `Subset`s:
...
| `drop_last` | `True` | `False` | A tiny trailing batch (e.g. 3 samples) gives a noisy gradient — and crashes BatchNorm on Day 7 if it's size 1. In eval, dropping samples would bias your metric |
...
## End to end: the pipeline meets yesterday's model

Everything assembled, plus a minimal consumption loop proving the pipe delivers. (The *proper* training loop — metrics, checkpointing, schedulers — is Day 4's whole agenda; this is deliberately bare.)
...
if __name__ == "__main__":          # required if you ever set num_workers > 0
    ...
    model = nn.Sequential(           # Day 2's MLP, sized for 8 features
...
One last structural point: notice that the training loop never mentions CSVs, normalization, splits, or shuffling. It sees only `for xb, yb in loader`. Swap `HousesDataset` for an image dataset tomorrow and the loop doesn't change a character. That interface stability is the payoff of today's work.
...
Tomorrow: the training loop, properly — train/eval modes, gradient accumulation, metric tracking, early stopping, and turning today's three-line loop into machinery you can trust.
