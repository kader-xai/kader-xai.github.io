---
aliases: ["/mini-courses/pytorch/day01.html"]
title: "🔥 Deep Learning with PyTorch · Lesson 1 — Tensors & Autograd: The Foundation"
page-layout: full
toc: true
toc-location: left
...
| `torch.float16` / `torch.bfloat16` | Mixed-precision training | Lesson 7 territory; halves memory. |
...
That's a complete machine learning system: model, loss, optimization — in ~20 lines of tensor code. Lessons 2–4 replace each piece with its industrial-strength counterpart (`nn.Linear`, `nn.MSELoss`, `torch.optim.SGD`), but the *mechanics* never change from what you just ran.
...
- The eternal loop: **forward → backward → update (no_grad) → zero**. Everything you train from here to Lesson 9 is this loop wearing better clothes.
...
In the next lesson: we stop hand-rolling `w * x + b` and let `nn.Module` and `nn.Linear` manage parameters for us — same math, real architecture.
