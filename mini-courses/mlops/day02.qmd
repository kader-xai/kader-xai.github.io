---
title: "🚢 ML in Production — MLOps · Day 2 — Reproducible Training: Seeds, Pins, and Config-as-Code"
page-layout: full
toc: true
toc-location: left
...
Yesterday we stared into the deployment gap: a churn model that lives in a notebook, scores 0.84 AUC on Tuesday and 0.81 on Thursday, and nobody can say why. Today we close the first and most fundamental part of that gap — **reproducibility**. Before a model can be tracked (Day 3), registered (Day 4), or shipped (Days 5–6), the same command on the same commit must produce the same model. That sounds obvious and is violated constantly, usually in four independent places: unseeded randomness, drifting dependency versions, hyperparameters buried in code, and data splits that silently shuffle. We will fix all four, and by the end of the day the messy Day-1 script will have become a clean, parametrized `train.py` you can run with `make train` — twice — and get byte-for-byte identical metrics.

::: {.callout-note appearance="simple"}
🎯 **Today you will:** seed every RNG in one function, pin your environment with a real lockfile, move hyperparameters into a validated YAML config, split data deterministically with hashing, and wire it all together with a Makefile and a refactored `train.py`
:::
...
Pin all four and training becomes a pure function of the git commit. That property is what everything else in this course leans on: experiment tracking is only meaningful if runs are comparable, and CI/CD (Day 8) can only gate a retrain if retraining is deterministic.
...
Our target layout for the day — this is what the Day-1 notebook becomes:
...
def set_seed(seed: int) -> None:
    ...
    try:
        ...
        pass  # sklearn-only project today; torch branch activates on Day 7's box
```
...
(If you'd rather source config from environment variables — say, per-deployment overrides — `pydantic-settings` gives you the same models with env-var loading layered in. Today, YAML + `model_validate` covers everything we need; we'll revisit env-driven settings when the FastAPI service appears on Day 6.)
...
`src/data.py` — the generator (our stand-in for Day 1's raw export, itself fully seeded) and the split:
...
- **The `salt` parameter** is your "re-roll" knob: the split is deterministic per `(id, salt)`, so if you ever legitimately need a *different* deterministic split — cross-validation folds, an uncontaminated holdout for a new model generation — change the salt, not the method. Today's exercise builds on this.
...
Now we assemble the pieces. Everything Day 1's notebook did — load, split, fit, evaluate, save — but as a pure function of `(config file, code commit)`, writing artifacts that record their own provenance:
...
A tiny helper with a big payoff: every artifact we write will carry the commit that produced it. On Day 3, MLflow takes over this bookkeeping — but the habit of *"no artifact without provenance"* starts now, and the fallback to `"unknown"` keeps the script runnable outside a repo (e.g., inside Day 5's Docker build).
...
`main()` is deliberately thin: parse two paths, delegate. The separation between `train(cfg, out_dir)` (pure-ish, testable, importable) and `main()` (CLI glue) is what lets Day 3 call `train()` from an MLflow-wrapped runner and Day 8 call it from a CI job without subprocess gymnastics. Run it:
...
Last piece. Every command we've typed today is a small liturgy (`uv run python -m src.train --config ...`) that teammates will mistype and CI will duplicate. A Makefile turns each *outcome* into one memorable verb — and on Day 8 the CI pipeline will simply call these same targets, so local and CI behavior can't drift apart.
...
flowchart TD
    ...
    C -.->|"Day 8: CI runs the same targets"| E(("✔"))
```
...
Tomorrow we stop stuffing provenance into hand-rolled JSON files: Day 3 puts MLflow in front of `train()` so every run's params, metrics, and artifacts are tracked, compared, and queryable.
