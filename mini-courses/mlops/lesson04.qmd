---
aliases: ["/mini-courses/mlops/day04.html"]
title: "🚢 ML in Production — MLOps · Lesson 4 — The Model Registry: Versioning and Promotion by Alias"
page-layout: full
toc: true
toc-location: left
...
In the previous lesson you instrumented training with MLflow: every run now logs its parameters, metrics, and a serialized model artifact. That solved *"which experiment produced this number?"* — but it created a new problem. You now have forty runs, and somewhere in that pile is *the* model: the one production should serve. Pointing production at a run ID is fragile (run IDs are opaque hashes nobody remembers) and pointing it at a file path is worse (paths change, files get overwritten). In this lesson we put a **model registry** between training and serving: a named, versioned catalog where `churn` version 7 is a real, addressable thing, `@champion` is a movable pointer to whichever version production trusts, and promotion is a one-line, auditable operation. By the end of this lesson you'll have a `promote.py` script that gates promotion on metrics — the exact script Lesson 8's CI/CD pipeline will call.

::: {.callout-note appearance="simple"}
🎯 **In this lesson you will:** register the churn model from a tracked run, manage versions with champion/challenger aliases instead of deprecated stages, load `models:/churn@champion` in inference code, trace any serving model back to its exact training run, write a metric-gated `promote.py`
:::

## Why a registry, and where it sits

The tracking server (Lesson 3) answers *"what happened?"* — it's a lab notebook. The registry answers *"what is official?"* — it's a release catalog. The distinction matters because the two have different lifecycles: runs are cheap and disposable (you'll log hundreds), registered versions are deliberate (you register only models that are candidates for production).
...
```{mermaid}
flowchart LR
    subgraph Tracking["Tracking server (Lesson 3)"]
        R1[run a1b2<br/>auc=0.81]
        R2[run c3d4<br/>auc=0.84]
        R3[run e5f6<br/>auc=0.86]
    end
    subgraph Registry["Model registry (current)"]
        V1[churn v1]
        V2[churn v2]
        V3[churn v3]
    end
    subgraph Serving["Serving (Lesson 6)"]
...
Two ways to get a model into the registry. The first is inline at training time — one extra argument to the `log_model` call you wrote in the previous lesson:

```python
# train.py (Lesson 2/3 file, one-line change)
import mlflow
...
with mlflow.start_run() as run:
    # ... fit pipeline, log params & metrics as in Lesson 3 ...
...
In practice: use `registered_model_name` in `train.py` when every training run is a candidate (typical for automated retraining, Lesson 10); use `mlflow.register_model` when a human or a CI job picks winners from many runs (typical for experimentation phases).
...
Here is the payoff. Any consumer — a batch scoring job, Lesson 6's FastAPI service, a notebook — loads the production model with one URI and zero knowledge of versions:
...
Chain it together: **serving alias → version → run → params/metrics/artifacts → git commit → code**. Combined with Lesson 2's reproducible training, that commit plus those params re-creates the model bit-for-bit. This chain is the whole reason we built Lessons 2–4 in this order.

## `promote.py` — metric-gated promotion

Promotion should never be "someone clicked a button in the UI on a Friday." It should be a script with an explicit gate, runnable by a human today and by CI on Lesson 8. The gate we'll use: the challenger must beat the champion's AUC by a margin $\epsilon$ on their (identical, Lesson 2-seeded) held-out split:
...
- **The gate exits nonzero on failure.** That's not pedantry: on Lesson 8 this script runs inside a pipeline, and the exit code *is* the interface. A gate that "fails" with exit 0 silently green-lights bad models.
...
The subtle assumption worth naming: comparing metrics from two different runs is only fair if both runs evaluated on the **same split**. Lesson 2's fixed seed and pinned data give us that. If your retraining data changes between runs (it will, by Lesson 10), the honest gate re-evaluates both models on one fresh holdout set — same script shape, just call `predict` on both loaded models instead of reading logged metrics. Keep that upgrade in your pocket for Lesson 10.
...
- Lineage is a chain: alias → version → run → params/metrics → git commit → (with DVC) exact data bytes. Every link exists after this lesson.
- Promotion is a script with a metric gate and a nonzero exit on failure — that exit code is the contract CI will rely on.

In the next lesson: the model leaves your laptop — we freeze its whole runtime into a Docker image and make "works on my machine" everyone's machine.
