---
aliases: ["/mini-courses/mlops/day07.html"]
title: "🚢 ML in Production — MLOps · Lesson 7 — Serving LLMs: vLLM in Practice"
page-layout: full
toc: true
toc-location: left
...
In the previous lesson you wrapped the churn model in FastAPI and it worked beautifully: load a pickle once at startup, run `predict_proba` in a few milliseconds, return JSON. In this lesson we hit the case where that recipe collapses. Our product team wants a new endpoint — `/explain` — that turns a churn prediction into a plain-English explanation a support agent can read to a customer. That means serving a large language model, and an LLM is not "a bigger sklearn model." It's an autoregressive generator whose cost, memory behavior, and batching semantics are so different that a whole class of specialized servers exists for it. In this lesson you'll stand up **vLLM**, the de-facto open-source standard, talk to it through the OpenAI-compatible API, understand *why* it's fast (continuous batching + PagedAttention), and learn the two knobs that trade throughput against latency. We'll close with the question every team should ask first: should you be self-hosting at all?

::: {.callout-note appearance="simple"}
🎯 **In this lesson you will:** launch an open model with `vllm serve`, call it from Python via the OpenAI client (including streaming), wire an LLM explanation endpoint into the Lesson 6 churn service, tune `--max-num-seqs` and `--gpu-memory-utilization` with a real benchmark, and run a quantized model on a small GPU
:::

## Why the Lesson 6 recipe fails for LLMs
...
Let's put numbers on point 3, because it drives everything else in this lesson. During generation, the model caches the key and value vectors of every past token in every layer (see the encyclopedia's *Transformers* and *Model Serving* chapters for the theory). Per token, that costs:
...
- **`Qwen/Qwen2.5-1.5B-Instruct`** — a HuggingFace repo ID. vLLM downloads weights on first launch (set `HF_HOME` to control the cache directory; in Docker you'll mount it as a volume, exactly like the model artifact volume from Lesson 5).
...
- **`--port 8001`** — our churn API from Lesson 6 already owns 8000.
...
Now the payoff: wire this into the Lesson 6 churn service. The architecture is two processes — the churn API stays a lean CPU service; vLLM runs as its own GPU service. Never load an LLM inside your FastAPI workers: it would couple your API's memory footprint and restart time to a multi-gigabyte model, and Lesson 5 taught us one concern per container.

```python
# app/explain.py -- add to the Lesson 6 FastAPI app
import os
...
class ExplainRequest(BaseModel):
    ...
    top_features: dict[str, float]  # feature -> value, from the Lesson 6 prediction
...
- **The cost crossover needs *your* utilization, not the GPU's spec sheet** — and the denominator must include the engineer-weeks that this lesson's lesson compressed into an afternoon, plus who wakes up when it pages.
...
# 1. Load the high-risk cohort (produced on Lesson 6)
...
- LLM serving breaks the Lesson 6 recipe because generation is sequential per request, request lengths are unpredictable, and the KV cache (~100+ KB/token for 8B-class models) dominates GPU memory.
...
In the next lesson we stop deploying by hand: CI/CD for ML — pipelines that test, build, and ship the churn service (and catch a bad model before it ships) on every push.
