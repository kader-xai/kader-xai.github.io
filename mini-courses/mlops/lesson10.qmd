---
aliases: ["/mini-courses/mlops/day10.html"]
title: "🚢 ML in Production — MLOps · Lesson 10 — Continuous Training: Closing the Loop"
page-layout: full
toc: true
toc-location: left
...
Run it against the drifted traffic you simulated on Lesson 9:
...
HOLDOUT_PATH = "data/holdout.csv"   # frozen on Lesson 2, never trained on
```
...
1. **Detect** — Lesson 9's dashboards: error rate, score distribution shift, or business metric drop after a deploy.
2. **Flip** — `python -m src.rollback` (seconds; no training, no build).
3. **Reload** — restart/redeploy the serving container so it re-resolves `@champion` (Lesson 8's deploy workflow, or `docker restart` locally). If you built Lesson 6's optional `/reload` admin endpoint, it's zero-downtime.
