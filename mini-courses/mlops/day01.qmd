---
title: "🚢 ML in Production — MLOps · Day 1 — The Deployment Gap & the Plan"
page-layout: full
toc: true
toc-location: left
...
Welcome to the course. Over the next ten days we take one model — a churn classifier we'll simply call *the model* — from a notebook-grade script all the way to a monitored, auto-retraining production service. Today is the foundation day: we'll understand *why* the distance between "my notebook says AUC 0.84" and "this thing serves predictions reliably at 3am" is where most ML projects die, we'll make the single most consequential architecture decision (batch vs online vs streaming serving), and we'll lay down the repo skeleton and a baseline training script that every subsequent day builds on. Nothing today is throwaway — the exact files you write in the next hour are the ones Day 8's CI pipeline will lint, Day 5's Dockerfile will copy, and Day 10's retraining job will invoke.

::: {.callout-note appearance="simple"}
🎯 **Today you will:** understand the deployment gap and where post-training effort actually goes, choose between batch/online/streaming serving with a concrete decision procedure, set up the full project repo layout for the course, write and run a baseline sklearn training script with a deterministic synthetic dataset, add a first smoke test
:::
...
- **The notebook lies about its environment.** "Works on my machine" for ML means: this Python, this sklearn version, this random seed, this CSV that happened to be on disk. None of that survives contact with a fresh server unless you make it survive (Days 2 and 5).
- **A prediction is a promise with an SLA.** Once another system calls your model, you own latency, throughput, uptime, input validation, and versioned behavior. `model.predict(X)` becomes an API contract (Day 6).
- **Nobody can answer "which model is this?"** Without tracking and a registry (Days 3–4), the production model is "whatever pickle Bob copied in March," and reproducing a bad prediction is archaeology.
...
<svg viewBox="0 0 700 360" xmlns="http://www.w3.org/2000/svg" role="img" aria-label="Iceberg of ML effort: model training above the waterline, everything else below">
  ...
  <!-- below labels -->
  <text x="350" y="140" text-anchor="middle" font-size="13" fill="currentColor">reproducibility · data versioning (Day 2)</text>
  <text x="350" y="170" text-anchor="middle" font-size="13" fill="currentColor">tracking · registry (Days 3–4)</text>
  <text x="350" y="200" text-anchor="middle" font-size="13" fill="currentColor">packaging · serving (Days 5–7)</text>
  <text x="350" y="230" text-anchor="middle" font-size="13" fill="currentColor">CI/CD · testing (Day 8)</text>
  <text x="350" y="260" text-anchor="middle" font-size="13" fill="currentColor">monitoring · drift (Day 9)</text>
  <text x="350" y="290" text-anchor="middle" font-size="13" fill="currentColor">continuous retraining (Day 10)</text>
</svg>

If you've read the encyclopedia's MLOps chapter, this is the practical companion: there we defined the concepts; here we build each layer of the iceberg with our own hands, one day at a time.
...
## The running example and the ten-day architecture
...
Here is the full system we'll have by Day 10. Keep this diagram in mind all week; each day lights up one or two boxes.

```{mermaid}
flowchart LR
    subgraph DEV["Days 1–2 · develop"]
        D[(training data)] --> T["train.py<br/>(reproducible)"]
    end
    subgraph TRACK["Days 3–4 · track & version"]
        T -->|params, metrics, artifacts| ML["MLflow<br/>tracking server"]
        ML --> REG["Model Registry<br/>churn-model @ v3 'production'"]
    end
    subgraph SHIP["Days 5–8 · package & serve"]
        REG --> IMG["Docker image"]
        IMG --> API["FastAPI service<br/>/predict"]
        CI["GitHub Actions CI/CD"] -.tests, build, deploy.-> IMG
    end
    subgraph RUN["Days 9–10 · operate"]
...
Everything lives in one repository. Create the skeleton now; we fill the empty directories on their respective days, but having them from Day 1 means the structure never needs refactoring mid-course.
...
touch Dockerfile .github/workflows/ci.yaml   # empty placeholders for Days 5 & 8
```
...
│       ├── __init__.py
│       ├── data.py       # data loading/generation (today)
│       └── train.py      # training entrypoint (today)
├── configs/
│   └── train.yaml        # all knobs live here, not in code (today)
├── tests/
│   └── test_train.py     # smoke test (today) → full suite Day 8
├── data/                 # local data cache — gitignored, DVC-managed Day 2
├── models/               # local model artifacts — gitignored, registry Day 4
├── Dockerfile            # Day 5
├── .github/workflows/
│   └── ci.yaml           # Day 8
├── pyproject.toml
...
- **`configs/` separate from code.** Every value someone might want to change without editing Python — data size, model hyperparameters, output paths — lives in YAML. On Day 3 MLflow will log this file wholesale; on Day 10 the retraining job will override parts of it. Hardcoded constants can't be logged or overridden.
...
Note the *floor* pins (`>=`) — good enough for Day 1. On Day 2 we'll generate a fully locked environment, because `>=` is exactly the kind of looseness that makes today's model unreproducible next month.
...
Time for the heart of the day. We build it in three stages: config, data, training — three small files instead of one monolith, because each will evolve independently (data gets DVC on Day 2, training gets MLflow on Day 3).
...
Every number that appears anywhere in training is here. When Day 10's retraining job wants a bigger dataset or Day 3's experiments sweep `C`, they edit *this*, not the code.

### Stage 2: deterministic data

Real churn projects pull from a warehouse; for a self-contained course we *generate* a realistic churn dataset. Crucially, we generate it **deterministically** — same seed, same bytes — which is a stand-in for the data-versioning discipline Day 2 formalizes.
...
- **The logit is the ground truth.** We define churn probability as a known linear function of features plus noise. This means we *know* the best achievable performance, which makes every later stage debuggable: if Day 9's monitoring reports AUC 0.65 when the data-generating process supports ~0.80, something in the pipeline broke — not the world.
- **Feature signs are realistic on purpose.** Tenure protects, support calls scream risk. On Day 9 we'll *shift these distributions deliberately* to simulate drift and watch the monitors fire.
...
- **`ColumnTransformer` names its columns.** `NUMERIC`/`CATEGORICAL`/`PASSTHROUGH` as module-level constants become the de-facto input schema. On Day 6, FastAPI's request model is derived from exactly these lists; on Day 9, the drift monitor iterates over them. Define the schema once.
...
- **`train()` takes a dict and returns a dict.** The `__main__` block is a thin shell around a pure function. That's deliberate: on Day 3 MLflow wraps `train()`, on Day 8 pytest calls it directly, on Day 10 the retraining job calls it with a modified config. Scripts whose logic lives under `if __name__ == "__main__":` can only ever be run, never composed.
- **Metrics printed as JSON**, not prose. A structured stdout contract means Day 8's CI can `json.loads` the output and gate the deploy on `roc_auc`.
...
An AUC around 0.81 against a known data-generating process with irreducible noise — a healthy, boring baseline. Resist the urge to gradient-boost it upward; a stronger model changes nothing about the next nine days, and the fixed baseline gives us a stable reference point for detecting regressions.

### Stage 4: the first test

A production repo without tests isn't a production repo. One smoke test today; Day 8 grows the suite.
...
```bash
git add -A && git commit -m "Day 1: repo skeleton, baseline training, smoke tests"
```
...
|---|---|---|
| `src/churn/data.py` | synthetic generator | DVC-versioned data stage (Day 2) |
| `src/churn/train.py` | prints JSON metrics | logs to MLflow (Day 3), registers models (Day 4), invoked by retraining (Day 10) |
| `configs/train.yaml` | local knobs | logged config artifact (Day 3), overridden by CT job (Day 10) |
| `models/*.joblib` | local file | registry version with stage labels (Day 4), baked into image (Day 5) |
| `Dockerfile` | empty | the production packaging (Day 5) |
| `.github/workflows/ci.yaml` | empty | test + build + deploy pipeline (Day 8) |
| `tests/` | 2 smoke tests | full pyramid: data, model, API contract tests (Day 8) |
...
Tomorrow: we make today's "it ran on my laptop" claim bulletproof — locked environments, versioned data with DVC, and a training run any machine can reproduce byte-for-byte.
