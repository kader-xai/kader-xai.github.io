---
aliases: ["/mini-courses/mlops/day06.html"]
title: "🚢 ML in Production — MLOps · Lesson 6 — Serving with FastAPI: a Typed Prediction API"
page-layout: full
toc: true
toc-location: left
...
In the previous lesson we packaged the churn model's world into a Docker image — same Python, same libraries, same code, everywhere. But an image that just *contains* a model is a filing cabinet, not a service. In this lesson we give the model a front door: an HTTP API that loads the registry champion from Lesson 4, validates every incoming request against a typed schema, predicts one customer or a thousand at a time, and tells operators whether it's alive and exactly which model version is answering. We'll build it with FastAPI and pydantic, prove it correct with a pytest test-client suite that needs no MLflow server and no real model, and then point a tiny load-test script at it to see what one process can actually do. This is the layer everything after this lesson builds on: Lesson 7 swaps the model for an LLM, Lesson 8 puts this API under CI/CD, and Lesson 9 bolts monitoring onto it.

::: {.callout-note appearance="simple"}
🎯 **In this lesson you will:** build a typed `/predict` endpoint with pydantic request/response schemas, load the registry champion at startup via lifespan, add batch/`/health`/`/model-info` endpoints with real error handling, run it under multiple uvicorn workers, and verify it with a pytest suite plus a load-test script.
:::
...
Project layout after this lesson (extending the repo from Lessons 2–5):

```
churn-service/
├── src/
│   ├── train.py          # Lesson 2–3
│   └── serve.py          # ← this lesson
├── tests/
│   └── test_serve.py     # ← this lesson
├── scripts/
│   └── load_test.py      # ← this lesson
├── Dockerfile            # Lesson 5, CMD updated in this lesson
└── requirements.txt      # + fastapi, uvicorn, httpx
```

Add the serving dependencies (pin them — Lesson 2's lesson still applies):
...
# The exact column order the model was trained on (Lesson 2's feature list).
FEATURES = [
    "tenure_months",
    "monthly_charges",
    "total_charges",
...
Lesson 4 left us with a registry model `churn-classifier` and an alias `@champion` pointing at the blessed version. The serving contract is: *this API serves whatever the champion alias points to, and can prove which version that is.*
...
- **If `load_champion()` raises, the worker refuses to start.** That's the behavior you want — crash-fast at deploy time beats serving 500s at request time. Your orchestrator (Docker healthcheck from Lesson 5, or Kubernetes later) sees the dead process and keeps the old version serving.
...
```bash
export MLFLOW_TRACKING_URI=http://127.0.0.1:5000   # Lesson 3's server
...
- **Memory**: total ≈ $W \times (\text{app} + \text{model})$. A 120 MB sklearn pipeline × 4 workers = ~500 MB before you serve a single request. For this lesson's model that's trivial; for a 5 GB model it's the reason Lesson 7 exists — you *cannot* scale LLM serving by forking copies, which is precisely why vLLM uses one engine with internal batching instead.
...
Update Lesson 5's Dockerfile `CMD` to match:
...
Sub-second, no network, no MLflow, no Docker. This exact suite becomes a CI gate on Lesson 8.
...
- A monkeypatched `load_champion` plus `TestClient` gives you a full-contract test suite with zero infrastructure — your Lesson 8 CI gate.

In the next lesson the per-worker-model-copy trick hits a wall: the model is a multi-gigabyte LLM, and we serve it properly with vLLM — one engine, continuous batching, and OpenAI-compatible endpoints.
