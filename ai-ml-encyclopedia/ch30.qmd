...
**QLoRA** combines this with low-rank adapters: freeze the nf4-quantized base, train only small LoRA matrices on top, and you can *[fine-tune](ch45.html)* a 13B model on one consumer GPU — the quantized base never needs gradients.
...
The temperature is the heart of it. A **[softmax](ch14.html) with temperature** $T$ computes $p_i = \dfrac{e^{z_i/T}}{\sum_j e^{z_j/T}}$. At $T=1$ it is the ordinary softmax; raising $T$ *flattens* the distribution, exposing the small probabilities ("dark knowledge") that tell the student how the teacher relates the wrong classes to each other. The $T^2$ factor on the KL term rescales its gradient back to the same magnitude as the hard-label term.
...
::: {.callout-note appearance="simple"}
**↪ The thread continues → [Chapter 31 · 🧰 Tools & Frameworks](ch31.html)**
