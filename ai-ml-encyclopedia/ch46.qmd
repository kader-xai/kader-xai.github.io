...
<style>
@keyframes c46-baton { 0%,100% { offset-distance: 0%; } 50% { offset-distance: 100%; } }
@keyframes c46-glow { 0%,100% { opacity: .35; } 50% { opacity: 1; } }
@media (prefers-reduced-motion: reduce) { .c46-runner { animation: none !important; } }
</style>
<svg viewBox="0 0 640 90" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="11" fill="currentColor">
  <line x1="40" y1="55" x2="600" y2="55" stroke="currentColor" stroke-dasharray="3 4" opacity="0.5"/>
  <g>
    <circle cx="60"  cy="55" r="11" fill="rgba(99,102,241,0.35)" stroke="currentColor"/><text x="60"  y="80" text-anchor="middle">Base</text>
    <circle cx="200" cy="55" r="11" fill="rgba(56,189,248,0.4)"  stroke="currentColor"/><text x="200" y="80" text-anchor="middle">SFT</text>
    <circle cx="360" cy="55" r="11" fill="rgba(34,197,94,0.4)"   stroke="currentColor"/><text x="360" y="80" text-anchor="middle">Preference</text>
    <circle cx="520" cy="55" r="11" fill="rgba(168,85,247,0.4)"  stroke="currentColor"/><text x="520" y="80" text-anchor="middle">Reasoning RL</text>
  </g>
  <circle class="c46-runner" r="6" fill="#f59e0b" style="offset-path: path('M60,55 L520,55'); animation: c46-baton 6s ease-in-out infinite;"/>
  <text x="320" y="22" text-anchor="middle" font-weight="bold">the baton: a model getting steadily more aligned</text>
</svg>
...
**Third**, optimize the policy $\pi_\theta$ with **PPO** ([Chapter 25](ch25.html)) to maximize reward, while a **KL penalty** chains it to the frozen reference (the SFT model) so it doesn't drift into gibberish that happens to fool the reward model:
...
<style>
@keyframes c46-think { 0% { offset-distance: 0%; } 100% { offset-distance: 100%; } }
@keyframes c46-pulse { 0%,100% { r: 5; opacity: .7; } 50% { r: 7; opacity: 1; } }
@media (prefers-reduced-motion: reduce) { .c46-dot { animation: none !important; } }
</style>
<svg viewBox="0 0 640 200" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="12" fill="currentColor">
  <text x="10" y="18" font-weight="bold">Test-time compute: more thinking → higher accuracy (log scale)</text>
  <line x1="50" y1="160" x2="610" y2="160" stroke="currentColor"/>
  <line x1="50" y1="160" x2="50"  y2="40"  stroke="currentColor"/>
  <text x="330" y="190" text-anchor="middle" font-style="italic">reasoning tokens (log budget) →</text>
  <text x="22" y="100" text-anchor="middle" font-style="italic" transform="rotate(-90 22 100)">accuracy →</text>
  <path id="c46-curve" d="M50,150 C180,150 250,70 360,60 C470,52 540,48 610,46" fill="none" stroke="#a855f7" stroke-width="2.5"/>
  <circle class="c46-dot" r="6" fill="#22c55e" stroke="currentColor" style="offset-path: path('M50,150 C180,150 250,70 360,60 C470,52 540,48 610,46'); animation: c46-think 7s ease-in-out infinite, c46-pulse 2s ease-in-out infinite;"/>
  <text x="470" y="40" fill="#a855f7">diminishing returns</text>
</svg>
...
- **Prompt injection** — hostile instructions hidden in *data* the model reads (a web page, a retrieved document, an email) that hijack its behavior. This is the dominant risk for agents and RAG systems ([Chapter 44](ch44.html)).
...
<style>
@keyframes c46-tilt { 0%,100% { transform: rotate(-9deg); } 50% { transform: rotate(9deg); } }
@media (prefers-reduced-motion: reduce) { .c46-beam { animation: none !important; } }
</style>
<svg viewBox="0 0 640 170" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="12" fill="currentColor">
  <text x="320" y="20" text-anchor="middle" font-weight="bold">The safety see-saw — you cannot floor both ends</text>
  <polygon points="320,140 300,150 340,150" fill="rgba(168,85,247,0.4)" stroke="currentColor"/>
  <g class="c46-beam" style="transform-origin: 320px 100px; animation: c46-tilt 6s ease-in-out infinite;">
    <line x1="120" y1="100" x2="520" y2="100" stroke="currentColor" stroke-width="3"/>
    <rect x="90"  y="78" width="120" height="24" rx="5" fill="rgba(236,72,153,0.35)" stroke="currentColor"/>
    <text x="150" y="95" text-anchor="middle">attack success</text>
    <rect x="430" y="78" width="120" height="24" rx="5" fill="rgba(245,158,11,0.4)" stroke="currentColor"/>
    <text x="490" y="95" text-anchor="middle">over-refusal</text>
  </g>
  <text x="320" y="165" text-anchor="middle" font-style="italic">tighten guardrails → safer but more false refusals; loosen → more helpful but more jailbreaks</text>
</svg>
...
The mindset shift mirrors MLOps ([Chapter 29](ch29.html)): evals are not a final exam you run once, they are a **regression suite** that grows with every bug you find and guards every future change.
...
::: {.callout-note appearance="simple"}
**↪ The thread continues → [Chapter 47 · 🚢 Model Serving & Deployment in Production](ch47.html)**
