...
**Deep Learning (DL)** is the subset of ML that uses **[neural networks](ch14.html) with many layers** to learn features automatically from raw data (pixels, audio, text), instead of you engineering those features by hand.
...
The pendulum swung to **connectionism** — learning from data with neural networks. The **perceptron** (1958) learned simple linear boundaries, but Minsky and Papert showed in 1969 it couldn't even learn XOR, triggering the first **AI winter** (a funding-and-interest collapse). **Backpropagation** (popularized 1986) let multi-layer networks learn, and **statistical ML** (SVMs, [random forests](ch10.html)) dominated the 1990s–2000s.

The modern era ignited in **2012**, when a deep [convolutional network](ch15.html) (AlexNet) crushed the ImageNet image-recognition benchmark, proving deep learning plus GPUs plus big data worked. The **[transformer](ch17.html)** (2017) unlocked scale, leading to **foundation models** — single large models pre-trained on internet-scale data, then adapted to many tasks ([LLMs](ch23.html) like GPT, BERT, and their descendants).

The "pendulum" isn't just a metaphor — here it is swinging, hype peaks giving way to winters and back again:

<style>
@keyframes c5-pendulum { 0%,100% { transform: rotate(-26deg); } 50% { transform: rotate(26deg); } }
@keyframes c5-glow { 0%,100% { opacity: 0.35; } 50% { opacity: 1; } }
@media (prefers-reduced-motion: reduce) {
  .c5-arm { animation: none !important; transform: rotate(0deg); }
  .c5-pulse { animation: none !important; opacity: 0.8; }
}
</style>
<svg viewBox="0 0 420 200" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif">
  <line x1="40" y1="170" x2="380" y2="170" stroke="currentColor" stroke-opacity="0.4"/>
  <text x="120" y="190" text-anchor="middle" font-size="10" fill="#6366f1" font-weight="bold">symbolic / rules</text>
  <text x="300" y="190" text-anchor="middle" font-size="10" fill="#22c55e" font-weight="bold">connectionist / learning</text>
  <circle cx="210" cy="30" r="5" fill="currentColor"/>
  <g class="c5-arm" style="transform-origin:210px 30px; animation:c5-pendulum 6s ease-in-out infinite;">
    <line x1="210" y1="30" x2="210" y2="135" stroke="currentColor" stroke-width="2"/>
    <circle cx="210" cy="145" r="14" fill="#f59e0b" fill-opacity="0.85" stroke="currentColor"/>
    <text x="210" y="149" text-anchor="middle" font-size="9" font-weight="bold" fill="#fff">AI</text>
  </g>
  <circle class="c5-pulse" cx="120" cy="120" r="6" fill="#6366f1" style="animation:c5-glow 6s ease-in-out infinite;"/>
  <circle class="c5-pulse" cx="300" cy="120" r="6" fill="#22c55e" style="animation:c5-glow 6s ease-in-out 3s infinite;"/>
</svg>
...
**Supervised learning** uses labeled examples — each input $x$ comes with the correct output $y$. The model learns a mapping $f: x \to y$. Two flavors: **[classification](ch09.html)** (discrete $y$, e.g. spam/not-spam) and **[regression](ch08.html)** (continuous $y$, e.g. house price). This is the workhorse of applied ML because labels make training direct.

**Unsupervised learning** has inputs but no labels. The goal is structure: **[clustering](ch11.html)** groups similar points, **[dimensionality reduction](ch07.html)** compresses them. You're asking "what's in here?" rather than "predict this."
...
**Reinforcement learning (RL)** has no fixed answer key at all. An **agent** takes actions in an environment and gets a **reward** signal, learning a **policy** that maximizes long-run reward by trial and error (see [Reinforcement Learning](ch25.html)).
...
**Problem framing** comes first and is the most-skipped step: decide what you're predicting, what a useful answer looks like, and the metric that defines success *in business terms* (a 1% accuracy gain is worthless if the cost of a false positive dominates). **Data** collection and cleaning typically eats most of the calendar. **Features** turn raw data into model inputs ([Data Preprocessing](ch06.html)). **Train** fits the model on a training split. **Evaluate** measures performance on held-out data ([Model Evaluation & Tuning](ch12.html)). **Deploy** puts it into production ([MLOps & Deployment](ch29.html)). **Monitor** watches for **drift** — the world changing so the model silently rots — which loops you back to data.
...
@keyframes c5-roll {
  0%   { offset-distance: 8%; }
  100% { offset-distance: 52%; }
}
@keyframes c5-spot { 0%,100% { r: 5; opacity: 0.6; } 50% { r: 8; opacity: 1; } }
@media (prefers-reduced-motion: reduce) {
  .c5-ball { animation: none !important; offset-distance: 52%; }
}
</style>
<svg viewBox="0 0 420 280" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif">
  ...
  <!-- rolling ball easing toward the bottom of the U -->
  <circle class="c5-ball" r="7" fill="#f59e0b" stroke="currentColor"
          style="offset-path:path('M60 95 Q200 130 220 125 Q300 120 390 70'); animation:c5-roll 5s ease-in-out infinite alternate;"/>
  <!-- sweet spot -->
  <line x1="215" y1="125" x2="215" y2="240" stroke="#888" stroke-dasharray="4 3"/>
  <circle cx="215" cy="125" r="5" fill="#2e7d32" style="animation:c5-spot 5s ease-in-out infinite;"/>
...
<svg viewBox="0 0 420 200" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif">
  <!-- L2 circle -->
  <g>
  <line x1="20" y1="170" x2="190" y2="170" stroke="currentColor" stroke-opacity="0.4"/>
  <line x1="105" y1="30" x2="105" y2="170" stroke="currentColor" stroke-opacity="0.4"/>
  <circle cx="105" cy="100" r="45" fill="rgba(56,189,248,0.3)" stroke="#38bdf8" stroke-width="2"/>
  <circle cx="150" cy="55" r="3.5" fill="#ec4899"/>
  <text x="105" y="190" text-anchor="middle" font-size="11" font-weight="bold">L2 (ridge): round</text>
  <text x="105" y="20" text-anchor="middle" font-size="9" fill="#666">touches off-axis → both weights small</text>
  </g>
  <!-- L1 diamond -->
  <g>
  <line x1="230" y1="170" x2="400" y2="170" stroke="currentColor" stroke-opacity="0.4"/>
  <line x1="315" y1="30" x2="315" y2="170" stroke="currentColor" stroke-opacity="0.4"/>
  <polygon points="315,55 360,100 315,145 270,100" fill="rgba(168,85,247,0.3)" stroke="#a855f7" stroke-width="2"/>
  <circle cx="315" cy="55" r="4" fill="#22c55e"/>
  <text x="315" y="190" text-anchor="middle" font-size="11" font-weight="bold">L1 (lasso): cornered</text>
  <text x="315" y="20" text-anchor="middle" font-size="9" fill="#666">touches a corner → one weight = 0</text>
  </g>
</svg>
...
- **[Data Preprocessing & Feature Engineering](ch06.html)** — the data and feature stages of the workflow in depth.
- **[Dimensionality Reduction](ch07.html)** — the cure for the curse of dimensionality.
- **[Regression](ch08.html)** and **[Classification Algorithms](ch09.html)** — the core supervised methods named here.
- **[Ensemble Methods](ch10.html)** — averaging away variance (bagging, boosting, random forests).
- **[Model Evaluation & Tuning](ch12.html)** — held-out testing, cross-validation, and tuning $\lambda$.
- **[Neural Networks (Core)](ch14.html)** — deep learning, dropout, and double descent.
- **[Reinforcement Learning](ch25.html)** — the agent/reward/policy paradigm in full.
- **[Frontier & Emerging Directions](ch39.html)** — foundation models and where the field is heading.

------------------------------------------------------------------------

::: {.callout-note appearance="simple"}
**↪ The thread continues → [Chapter 06 · 🧹 Data Preprocessing](ch06.html)**
