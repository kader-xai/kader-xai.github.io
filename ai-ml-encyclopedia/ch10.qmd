...
::: {.callout-warning}
Always calibrate on data the base model did **not** train on — reusing training rows makes the scores look perfectly calibrated in-sample and silently wrong in production, the same leakage trap as stacking (§10.9). And remember calibration changes the *probabilities*, not the *ranking*: it leaves AUC untouched while fixing metrics like log-loss and Brier score that actually depend on the numbers.
:::
