...
A single decision tree is a weak student: it overfits, it's jumpy, and its answer can flip if you nudge the data. Ensemble methods are the trick that turns a crowd of mediocre models into one strong predictor — and they are still, in 2026, the algorithms that win most competitions and power most production systems on tabular data. This chapter sits in Classical Machine Learning and builds directly on [decision trees](ch09.html): it is the bridge from "one model" to "many models that vote, average, or correct each other."
...
M, p = 100, 0.60
print("crowd error:", round(binom.sf(M//2 - 1, M, 1-p), 4))  # ~0.0176 -> 98%+ correct
```

The catch hidden in "if their mistakes are independent" is the whole game: real models trained on the same data make *correlated* mistakes, so the crowd never reaches this idealized 98%. Every technique in this chapter is, at bottom, a different way to make the members disagree more.
...
<svg viewBox="0 0 420 150" xmlns="http://www.w3.org/2000/svg">
  <line x1="45" y1="120" x2="400" y2="120" stroke="currentColor" stroke-opacity=".4"/>
  <line x1="45" y1="20" x2="45" y2="120" stroke="currentColor" stroke-opacity=".4"/>
  <text x="6" y="26" font-size="9" fill="currentColor" opacity=".7">100%</text>
  <text x="16" y="118" font-size="9" fill="currentColor" opacity=".7">0%</text>
  <!-- single-voter error baseline 40% -->
  <line x1="45" y1="72" x2="400" y2="72" stroke="#f59e0b" stroke-width="1.5" stroke-dasharray="4 3"/>
  <text x="300" y="68" font-size="9" fill="#f59e0b">one voter: 40% wrong</text>
  <!-- crowd error curve dropping toward 0 -->
  <polyline points="60,72 110,55 160,42 210,33 260,27 310,24 360,22 400,21"
            fill="none" stroke="#22c55e" stroke-width="2.5"/>
  <text x="120" y="40" font-size="9" fill="#22c55e">majority-vote error ↓</text>
  <text x="120" y="138" font-size="9" fill="currentColor" opacity=".7">number of independent voters →</text>
</svg>

To make this precise we need the **bias–variance decomposition** (its full [probability and statistics](ch04.html) derivation lives elsewhere). The expected prediction error of a model splits into three pieces:
...
<svg viewBox="0 0 440 130" xmlns="http://www.w3.org/2000/svg">
  <style>
    .ring { fill:none; stroke:currentColor; stroke-opacity:.35; }
    .lbl { font-size:9px; fill:currentColor; opacity:.75; }
  </style>
  <!-- board 1: low bias low var -->
  <g transform="translate(55,55)">
    <circle class="ring" r="34"/><circle class="ring" r="20"/><circle class="ring" r="7"/>
    <circle cx="2" cy="-1" r="3" fill="#22c55e"/><circle cx="-2" cy="3" r="3" fill="#22c55e"/>
    <circle cx="3" cy="2" r="3" fill="#22c55e"/><circle cx="-1" cy="-3" r="3" fill="#22c55e"/>
  </g>
  <text class="lbl" x="20" y="108">low bias · low var</text>
  <!-- board 2: high bias low var -->
  <g transform="translate(165,55)">
    <circle class="ring" r="34"/><circle class="ring" r="20"/><circle class="ring" r="7"/>
    <circle cx="20" cy="-16" r="3" fill="#f59e0b"/><circle cx="24" cy="-12" r="3" fill="#f59e0b"/>
    <circle cx="21" cy="-19" r="3" fill="#f59e0b"/><circle cx="26" cy="-15" r="3" fill="#f59e0b"/>
  </g>
  <text class="lbl" x="130" y="108">high bias · low var</text>
  <!-- board 3: low bias high var -->
  <g transform="translate(275,55)">
    <circle class="ring" r="34"/><circle class="ring" r="20"/><circle class="ring" r="7"/>
    <circle cx="-22" cy="10" r="3" fill="#38bdf8"/><circle cx="18" cy="-18" r="3" fill="#38bdf8"/>
    <circle cx="6" cy="22" r="3" fill="#38bdf8"/><circle cx="-12" cy="-20" r="3" fill="#38bdf8"/>
  </g>
  <text class="lbl" x="240" y="108">low bias · high var</text>
  <!-- board 4: high bias high var -->
  <g transform="translate(385,55)">
    <circle class="ring" r="34"/><circle class="ring" r="20"/><circle class="ring" r="7"/>
    <circle cx="14" cy="-24" r="3" fill="#ec4899"/><circle cx="26" cy="-4" r="3" fill="#ec4899"/>
    <circle cx="30" cy="12" r="3" fill="#ec4899"/><circle cx="8" cy="-12" r="3" fill="#ec4899"/>
  </g>
  <text class="lbl" x="350" y="108">high bias · high var</text>
</svg>
...
In practice you rarely hand-roll the bootstrap loop — scikit-learn's `BaggingRegressor`/`BaggingClassifier` wraps any base estimator and does the resampling, OOB scoring, and parallelism for you:
...
pred = bag.predict(X)
print("OOB R²:", round(bag.oob_score_, 3))   # free validation from the left-out 37%
```
...
# same API as RandomForestClassifier; usually trains noticeably faster
```

The picture: a random forest scans a feature and places the fence at the *best* threshold it can find; Extra Trees just throw the fence down at a *random* spot. Sloppier per tree, but no two trees place their fences alike — and that disagreement is exactly what the averaging cashes in.

<svg viewBox="0 0 420 130" xmlns="http://www.w3.org/2000/svg">
  <text x="20" y="16" font-size="10" fill="currentColor" opacity=".75">Random Forest: best threshold</text>
  <line x1="20" y1="95" x2="180" y2="95" stroke="currentColor" stroke-opacity=".35"/>
  <circle cx="40" cy="88" r="3" fill="#22c55e"/><circle cx="55" cy="90" r="3" fill="#22c55e"/>
  <circle cx="70" cy="86" r="3" fill="#22c55e"/><circle cx="130" cy="88" r="3" fill="#ec4899"/>
  <circle cx="150" cy="90" r="3" fill="#ec4899"/><circle cx="165" cy="85" r="3" fill="#ec4899"/>
  <line x1="100" y1="60" x2="100" y2="105" stroke="#6366f1" stroke-width="2"/>
  <text x="78" y="55" font-size="8" fill="#6366f1">optimal cut</text>
  <text x="240" y="16" font-size="10" fill="currentColor" opacity=".75">Extra Trees: random threshold</text>
  <line x1="240" y1="95" x2="400" y2="95" stroke="currentColor" stroke-opacity=".35"/>
  <circle cx="260" cy="88" r="3" fill="#22c55e"/><circle cx="275" cy="90" r="3" fill="#22c55e"/>
  <circle cx="290" cy="86" r="3" fill="#22c55e"/><circle cx="350" cy="88" r="3" fill="#ec4899"/>
  <circle cx="370" cy="90" r="3" fill="#ec4899"/><circle cx="385" cy="85" r="3" fill="#ec4899"/>
  <line x1="332" y1="60" x2="332" y2="105" stroke="#f59e0b" stroke-width="2" stroke-dasharray="4 3"/>
  <line x1="305" y1="60" x2="305" y2="105" stroke="#f59e0b" stroke-width="1" stroke-opacity=".5" stroke-dasharray="3 3"/>
  <line x1="360" y1="60" x2="360" y2="105" stroke="#f59e0b" stroke-width="1" stroke-opacity=".5" stroke-dasharray="3 3"/>
  <text x="300" y="55" font-size="8" fill="#f59e0b">a random cut (one of many)</text>
</svg>
...
<svg viewBox="0 0 420 160" xmlns="http://www.w3.org/2000/svg">
  <style>
    @keyframes c10-boost-roll {
      0%   { transform: translate(0px,0px); }
      100% { transform: translate(285px,98px); }
    }
    @keyframes c10-boost-fade { 0%,100% { opacity:.5; } 50% { opacity:1; } }
    .c10-ball { animation: c10-boost-roll 5s cubic-bezier(.45,.05,.55,.95) infinite alternate; }
    .c10-loss-lbl { animation: c10-boost-fade 5s ease-in-out infinite; }
    @media (prefers-reduced-motion: reduce) { .c10-ball { animation: none; } }
  </style>
  <line x1="35" y1="135" x2="405" y2="135" stroke="currentColor" stroke-opacity=".35"/>
  <line x1="35" y1="15" x2="35" y2="135" stroke="currentColor" stroke-opacity=".35"/>
  <text x="6" y="22" font-size="9" fill="currentColor" opacity=".7">loss</text>
  <text x="300" y="150" font-size="9" fill="currentColor" opacity=".7">rounds →</text>
  <!-- loss "bowl" the ball rolls down -->
  <path d="M55,30 Q150,40 200,95 T345,128" fill="none" stroke="#6366f1" stroke-width="2.5"/>
  <g class="c10-ball"><circle cx="55" cy="30" r="6" fill="#f59e0b"/></g>
  <text class="c10-loss-lbl" x="250" y="60" font-size="9" fill="#6366f1">each round = one step downhill</text>
</svg>
...
The same model in scikit-learn is a single class, with the modern histogram-based variant being the fast one to prefer:
...
print("stopped at", gb.n_iter_, "trees")
```
...
In practice you almost always pair these with **early stopping** on a held-out set so the library, not you, decides how many trees to grow:
...
print("best #trees:", model.best_iteration_)   # stops when val loss stalls
```
...
<svg viewBox="0 0 420 170" xmlns="http://www.w3.org/2000/svg">
  <style>
    @keyframes c10-stack-flow { 0% { offset-distance:0%; opacity:0; } 12% { opacity:1; } 88% { opacity:1; } 100% { offset-distance:100%; opacity:0; } }
    .c10-pulse { fill:#38bdf8; }
    .c10-w1 { offset-path: path("M70,40 L200,85"); animation: c10-stack-flow 3.2s linear infinite; }
    .c10-w2 { offset-path: path("M70,85 L200,85"); animation: c10-stack-flow 3.2s linear infinite .4s; }
    .c10-w3 { offset-path: path("M70,130 L200,85"); animation: c10-stack-flow 3.2s linear infinite .8s; }
    .c10-w4 { offset-path: path("M255,85 L350,85"); fill:#a855f7; animation: c10-stack-flow 3.2s linear infinite 1.4s; }
    @media (prefers-reduced-motion: reduce) { .c10-w1,.c10-w2,.c10-w3,.c10-w4 { animation: none; } }
  </style>
  <line x1="70" y1="40" x2="200" y2="85" stroke="currentColor" stroke-opacity=".3"/>
  <line x1="70" y1="85" x2="200" y2="85" stroke="currentColor" stroke-opacity=".3"/>
  <line x1="70" y1="130" x2="200" y2="85" stroke="currentColor" stroke-opacity=".3"/>
  <line x1="255" y1="85" x2="350" y2="85" stroke="currentColor" stroke-opacity=".3"/>
  <g font-size="9" fill="currentColor">
    <rect x="20" y="30" width="50" height="20" rx="4" fill="#22c55e" fill-opacity=".25" stroke="#22c55e"/><text x="28" y="44">forest</text>
    <rect x="20" y="75" width="50" height="20" rx="4" fill="#f59e0b" fill-opacity=".25" stroke="#f59e0b"/><text x="26" y="89">XGBoost</text>
    <rect x="20" y="120" width="50" height="20" rx="4" fill="#ec4899" fill-opacity=".25" stroke="#ec4899"/><text x="30" y="134">logreg</text>
    <rect x="200" y="72" width="55" height="26" rx="4" fill="#6366f1" fill-opacity=".25" stroke="#6366f1"/><text x="206" y="89">meta</text>
    <rect x="350" y="74" width="48" height="22" rx="4" fill="currentColor" fill-opacity=".12" stroke="currentColor" stroke-opacity=".5"/><text x="356" y="89">answer</text>
  </g>
  <circle class="c10-pulse c10-w1" r="3.5"/><circle class="c10-pulse c10-w2" r="3.5"/>
  <circle class="c10-pulse c10-w3" r="3.5"/><circle class="c10-pulse c10-w4" r="3.5"/>
</svg>
...
<svg viewBox="0 0 300 220" xmlns="http://www.w3.org/2000/svg">
  <line x1="40" y1="190" x2="270" y2="190" stroke="currentColor" stroke-opacity=".4"/>
  <line x1="40" y1="20"  x2="40"  y2="190" stroke="currentColor" stroke-opacity=".4"/>
  <text x="120" y="210" font-size="9" fill="currentColor" opacity=".7">predicted probability</text>
  <text x="34" y="16" font-size="9" fill="currentColor" opacity=".7">actual</text>
  <!-- perfect diagonal -->
  <line x1="40" y1="190" x2="270" y2="20" stroke="currentColor" stroke-dasharray="4 3" stroke-opacity=".5"/>
  <text x="200" y="55" font-size="8" fill="currentColor" opacity=".6">perfect</text>
  <!-- forest: sags below mid (under-confident) -->
  <polyline points="40,190 92,165 144,140 196,90 270,20" fill="none" stroke="#22c55e" stroke-width="2"/>
  <text x="150" y="158" font-size="8" fill="#22c55e">forest (under-confident)</text>
  <!-- boosting: S pushed to corners (over-confident) -->
  <polyline points="40,190 92,185 144,105 196,35 270,20" fill="none" stroke="#f59e0b" stroke-width="2"/>
  <text x="55" y="120" font-size="8" fill="#f59e0b">boosting (over-confident)</text>
</svg>
...
p = cal.predict_proba(Xcal)[:, 1]
```

::: {.callout-warning}
Always calibrate on data the base model did **not** train on — reusing training rows makes the scores look perfectly calibrated in-sample and silently wrong in production, the same leakage trap as stacking (§10.9). And remember calibration changes the *probabilities*, not the *ranking*: it leaves AUC untouched while fixing metrics like log-loss and Brier score (see [Model Evaluation & Tuning](ch12.html)) that actually depend on the numbers.
:::
...
<svg viewBox="0 0 420 170" xmlns="http://www.w3.org/2000/svg">
  <rect x="30" y="25" width="160" height="120" fill="none" stroke="currentColor" stroke-opacity=".4"/>
  <!-- dense cluster -->
  <circle cx="80" cy="100" r="3" fill="#6366f1"/><circle cx="90" cy="110" r="3" fill="#6366f1"/>
  <circle cx="100" cy="95" r="3" fill="#6366f1"/><circle cx="95" cy="120" r="3" fill="#6366f1"/>
  <circle cx="110" cy="108" r="3" fill="#6366f1"/><circle cx="85" cy="92" r="3" fill="#6366f1"/>
  <!-- outlier -->
  <circle cx="165" cy="45" r="4" fill="#ec4899"/>
  <!-- one slice isolates the outlier fast -->
  <line x1="145" y1="25" x2="145" y2="145" stroke="#ec4899" stroke-dasharray="3 2"/>
  <line x1="145" y1="60" x2="190" y2="60" stroke="#ec4899" stroke-dasharray="3 2"/>
  <text x="150" y="38" font-size="8" fill="#ec4899">2 cuts → alone</text>
  <text x="55" y="160" font-size="8" fill="#6366f1">crowd needs many cuts</text>
  <!-- score axis -->
  <text x="240" y="60" font-size="9" fill="currentColor" opacity=".75">short path  → anomaly</text>
  <text x="240" y="100" font-size="9" fill="currentColor" opacity=".75">long path   → normal</text>
</svg>
...
print("flagged anomalies:", int((pred == -1).sum()))
```

Because it only does a handful of random cuts per tree and never computes distances, an isolation forest is **fast and scales to high dimensions** — exactly where distance-based outlier methods (k-NN, LOF) bog down. It is a workhorse for [fraud screening](ch27.html), network-intrusion detection, sensor fault-finding, and data-quality checks before training.
...
- **Click-through-rate and ranking.** Ad and search systems rank candidates with boosted trees (LightGBM ships a `LambdaRank` objective built for exactly this). Millions of sparse features, sub-millisecond inference, retrained nightly.
- **Insurance pricing and risk.** Actuarial models on claim frequency and severity lean on boosting for accuracy plus monotonic constraints (force "older car ⇒ higher premium" to hold).
...
- **Images, audio, and raw text.** These have spatial or sequential structure that [convolutional](ch15.html) and [transformer](ch17.html) networks exploit; trees see only a flat vector and lose. Use [deep learning](ch14.html) (Chapters 14–18).
...
::: {.callout-note appearance="simple"}
**↪ The thread continues → [Chapter 11 · 🔮 Clustering & Unsupervised Learning](ch11.html)**
