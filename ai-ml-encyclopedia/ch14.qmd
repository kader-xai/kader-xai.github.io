...
In a framework the optimizer owns this update; you only supply gradients (via `backward()`) and call `step()`. Plain SGD *is* the rule above; momentum and Adam (next chapter) refine *which direction and how far* using the history of gradients, but the skeleton is identical:
