...
A single neuron, opened up, is just a dot product followed by a squashing function — the same shape as [logistic regression](ch09.html). Picture it as a little voting machine: each input arrives weighted, the votes are summed, a bias tips the scale, and the activation decides how loudly the neuron fires. The doodle below pulses to show a neuron gathering its inputs and "firing":

<style>
@keyframes c14-pulse { 0%,100% { r: 16; opacity: .55; } 50% { r: 21; opacity: .95; } }
@keyframes c14-spark { 0% { stroke-dashoffset: 60; opacity: 0; } 40%,100% { stroke-dashoffset: 0; opacity: .8; } }
@media (prefers-reduced-motion: reduce) { .c14-pulse-core, .c14-spark { animation: none; } }
</style>
<svg viewBox="0 0 300 130" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="11">
  <g class="c14-spark" stroke="#38bdf8" stroke-width="2" stroke-dasharray="60" style="animation: c14-spark 3s ease-in-out infinite">
    <line x1="30" y1="30" x2="120" y2="65"/><line x1="30" y1="65" x2="120" y2="65"/><line x1="30" y1="100" x2="120" y2="65"/>
  </g>
  <g fill="#38bdf8" opacity="0.8"><circle cx="30" cy="30" r="7"/><circle cx="30" cy="65" r="7"/><circle cx="30" cy="100" r="7"/></g>
  <circle class="c14-pulse-core" cx="135" cy="65" r="16" fill="#6366f1" style="animation: c14-pulse 3s ease-in-out infinite"/>
  <text x="135" y="69" text-anchor="middle" fill="currentColor" font-size="12">Σ</text>
  <line x1="155" y1="65" x2="210" y2="65" stroke="#22c55e" stroke-width="2" class="c14-spark" stroke-dasharray="60" style="animation: c14-spark 3s ease-in-out infinite; animation-delay: .6s"/>
  <circle cx="225" cy="65" r="9" fill="#22c55e" opacity="0.7"/>
  <text x="262" y="69" fill="currentColor">fires</text>
</svg>
...
A layer is fully described by a weight [matrix](ch01.html) $W$ and bias vector $b$. If layer $\ell$ has $n_\ell$ neurons and the previous layer has $n_{\ell-1}$ outputs, then $W^{[\ell]}$ has shape $n_\ell \times n_{\ell-1}$ and $b^{[\ell]}$ has length $n_\ell$.
...
Training means adjusting the weights to reduce the loss. To do that we need the **gradient** of the loss with respect to every weight. **Backpropagation** is the algorithm that computes those gradients efficiently by applying the [**chain rule** of calculus](ch02.html), working *backward* from the output to the input. The intuition: each weight's influence on the loss flows through the layers above it, so we compute the error at the output, then pass it back layer by layer, multiplying by local derivatives.
...
1. **Start the blame at the output.** $\delta^{[L]} = \dfrac{\partial L}{\partial a^{[L]}} \odot g'(z^{[L]})$ — how wrong the final answer was, scaled by how steep the output activation was.
2. **Pass the blame down a layer.** $\delta^{[\ell]} = (W^{[\ell+1]\top}\delta^{[\ell+1]}) \odot g'(z^{[\ell]})$ — pull the blame above back through the weights, then scale by this layer's slope.
3. **Read off the weight gradients.** $\dfrac{\partial L}{\partial W^{[\ell]}} = \delta^{[\ell]}\,a^{[\ell-1]\top}$ and $\dfrac{\partial L}{\partial b^{[\ell]}} = \delta^{[\ell]}$ — a weight's gradient is its incoming blame times the input that fed it.

The animation shows the blame flowing right-to-left, brightening each layer in turn:

<style>
@keyframes c14-blame { 0%,100% { opacity: .25; } 50% { opacity: 1; } }
@media (prefers-reduced-motion: reduce) { .c14-blame { animation: none !important; opacity: .8 !important; } }
</style>
<svg viewBox="0 0 380 90" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="11">
  <g stroke="#ec4899" stroke-width="1.4" opacity="0.5"><line x1="280" y1="45" x2="220" y2="45"/><line x1="190" y1="45" x2="130" y2="45"/><line x1="100" y1="45" x2="40" y2="45"/></g>
  <g text-anchor="middle" fill="currentColor">
    <circle class="c14-blame" cx="310" cy="45" r="20" fill="#ec4899" style="animation: c14-blame 3.2s ease-in-out infinite; animation-delay: 0s"/><text x="310" y="49">δᴸ</text>
    <circle class="c14-blame" cx="205" cy="45" r="20" fill="#a855f7" style="animation: c14-blame 3.2s ease-in-out infinite; animation-delay: .9s"/><text x="205" y="49">δ²</text>
    <circle class="c14-blame" cx="115" cy="45" r="20" fill="#6366f1" style="animation: c14-blame 3.2s ease-in-out infinite; animation-delay: 1.8s"/><text x="115" y="49">δ¹</text>
  </g>
  <text x="190" y="82" text-anchor="middle" fill="currentColor" font-size="9">blame flows backward: output → hidden → input</text>
</svg>
...
<svg viewBox="0 0 460 150" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="11">
  <g stroke="currentColor" stroke-width="0.9" opacity="0.7" fill="none" marker-end="url(#ah)">
    <line x1="58" y1="55" x2="98" y2="55"/>
    <line x1="142" y1="55" x2="182" y2="55"/>
    <line x1="226" y1="55" x2="266" y2="55"/>
    <line x1="310" y1="55" x2="350" y2="55"/>
  </g>
  <defs><marker id="ah" markerWidth="7" markerHeight="7" refX="5" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 z" fill="currentColor"/></marker></defs>
  <circle cx="40" cy="55" r="16" fill="#38bdf8" opacity="0.5" stroke="currentColor" stroke-width="0.7"/>
  <text x="40" y="59" text-anchor="middle" fill="currentColor">x·w₁</text>
  <circle cx="120" cy="55" r="16" fill="#6366f1" opacity="0.5" stroke="currentColor" stroke-width="0.7"/>
  <text x="120" y="59" text-anchor="middle" fill="currentColor">·w₂</text>
  <circle cx="204" cy="55" r="16" fill="#22c55e" opacity="0.5" stroke="currentColor" stroke-width="0.7"/>
  <text x="204" y="59" text-anchor="middle" fill="currentColor">−y</text>
  <circle cx="288" cy="55" r="16" fill="#f59e0b" opacity="0.5" stroke="currentColor" stroke-width="0.7"/>
  <text x="288" y="59" text-anchor="middle" fill="currentColor">(·)²</text>
  <circle cx="368" cy="55" r="16" fill="#ec4899" opacity="0.5" stroke="currentColor" stroke-width="0.7"/>
  <text x="368" y="59" text-anchor="middle" fill="currentColor">½·</text>
  <text x="404" y="59" fill="currentColor">L</text>
  <text x="40" y="95" text-anchor="middle" fill="currentColor" font-size="9">a₁</text>
  <text x="120" y="95" text-anchor="middle" fill="currentColor" font-size="9">ŷ</text>
  <g opacity="0.8"><text x="230" y="128" fill="#d9774a" font-size="10">forward →   backward ←  (each node knows its own local derivative)</text></g>
</svg>
...
**In words:** record what operations produced the loss; to differentiate, replay them in reverse, multiplying each node's local derivative into the running gradient.
**Also written:** for a chain $L = f_n(\dots f_2(f_1(w)))$, reverse-mode computes $\frac{\partial L}{\partial w} = \prod_{k} f_k'$ by accumulating the product from $f_n$ down to $f_1$ — exactly the $\delta$ recursion of §14.3.
...
::: {.callout-tip}
Default to ReLU for hidden layers in plain MLPs and CNNs; reach for GELU in [Transformers](ch17.html). Save sigmoid for a single output that must be a probability, not for hidden layers.
:::
...
**Where it shows up:** every time a [language model](ch23.html) picks the next word, a softmax over the whole vocabulary turns the model's raw scores into "probability of each possible next token" — and the *temperature* knob you see in chat APIs simply divides the logits before this softmax, flattening or sharpening that distribution to make outputs more random or more focused.
...
For [**regression**](ch08.html) (predicting a number), use **mean squared error (MSE)**: $L = \tfrac1N\sum (\hat y - y)^2$. It penalizes large errors quadratically and pairs naturally with a linear output.
...
For **classification**, use **cross-entropy**, which measures the distance between the predicted [probability distribution](ch04.html) and the true one. For $K$ classes with one-hot target $y$: $L = -\sum_{i} y_i \log \hat y_i$. Because the truth is one-hot, this reduces to $-\log \hat y_{\text{correct}}$ — the loss is just the negative log-probability the model assigned to the right answer.
...
@keyframes c14-roll {
  0% { offset-distance: 4%; }
  85% { offset-distance: 50%; }
  100% { offset-distance: 50%; }
}
@media (prefers-reduced-motion: reduce) { .c14-ball { animation: none; offset-distance: 50%; } }
</style>
<svg viewBox="0 0 320 150" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="11">
  <path d="M20,20 Q160,200 300,20" fill="none" stroke="currentColor" stroke-width="1.6" opacity="0.5"/>
  <circle class="c14-ball" r="8" fill="#22c55e"
    style="offset-path: path('M20,20 Q160,200 300,20'); animation: c14-roll 4s ease-in-out infinite"/>
  <text x="160" y="130" text-anchor="middle" fill="currentColor" font-size="10">each step rolls a little further downhill</text>
</svg>

$$w \leftarrow w - \eta\,\nabla_w L$$

**In words:** nudge each weight a little in the downhill direction; how far is set by the learning rate.
**Also written:** $w_{t+1} = w_t - \eta\,\dfrac{\partial L}{\partial w}\Big|_{w_t}$ — the same step, indexed by iteration $t$.
...
<svg viewBox="0 0 460 170" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="10">
  <!-- three identical loss bowls -->
  <g fill="none" stroke="currentColor" stroke-width="1.4" opacity="0.5">
    <path d="M10,20 Q70,160 130,20"/>
    <path d="M165,20 Q225,160 285,20"/>
    <path d="M320,20 Q380,160 440,20"/>
  </g>
  <!-- too small: tiny steps clustered high on left wall -->
  <g fill="#f59e0b"><circle cx="38" cy="92" r="3.5"/><circle cx="46" cy="74" r="3.5"/><circle cx="52" cy="60" r="3.5"/></g>
  <text x="70" y="150" text-anchor="middle" fill="currentColor">η too small</text>
  <!-- just right: steps marching into the basin -->
  <g fill="#22c55e"><circle cx="180" cy="58" r="3.5"/><circle cx="205" cy="100" r="3.5"/><circle cx="222" cy="120" r="3.5"/><circle cx="225" cy="126" r="3.5"/></g>
  <text x="225" y="150" text-anchor="middle" fill="currentColor">η just right</text>
  <!-- too large: zig-zag bouncing up the far wall -->
  <g fill="#ec4899"><circle cx="338" cy="64" r="3.5"/><circle cx="420" cy="46" r="3.5"/><circle cx="332" cy="38" r="3.5"/></g>
  <path d="M338,64 L420,46 L332,38" fill="none" stroke="#ec4899" stroke-width="1" opacity="0.7"/>
  <text x="380" y="150" text-anchor="middle" fill="currentColor">η too large</text>
</svg>
...
In a framework the optimizer owns this update; you only supply gradients (via `backward()`) and call `step()`. Plain SGD *is* the rule above; momentum and Adam (covered in [Optimization](ch03.html)) refine *which direction and how far* using the history of gradients, but the skeleton is identical:
...
- **Layer normalization** computes $\mu,\sigma^2$ across the **feature dimension** (per example, over its own activations). It's independent of batch size and identical at train and test time, which is why Transformers and [RNNs](ch16.html) use it.
...
- [**Calculus & Differentiation**](ch02.html) — the chain rule that backpropagation is built on.
- [**Optimization**](ch03.html) — SGD, momentum, Adam, and learning-rate schedules that drive the update step.
- [**Convolutional Neural Networks**](ch15.html) — feedforward networks specialized for images, where BatchNorm shines.
- [**Recurrent & Sequence Models**](ch16.html) — where vanishing gradients and gradient clipping matter most.
- [**Attention & Transformers**](ch17.html) — built from these layers plus LayerNorm, GELU, and residual connections.
- [**Model Evaluation & Tuning**](ch12.html) — how to choose batch size, learning rate, and stopping via validation.

------------------------------------------------------------------------

::: {.callout-note appearance="simple"}
**↪ The thread continues → [Chapter 15 · 🖼️ Convolutional Neural Networks](ch15.html)**
