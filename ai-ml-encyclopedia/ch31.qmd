...
- **`transformers`** — thousands of pretrained models (BERT, GPT-style, ViT, Whisper) behind one uniform API, plus the high-level `pipeline` that bundles tokenizer + model + post-processing into a single callable.
...
When you need control, drop one level to the tokenizer + model pair. The tokenizer turns text into the integer IDs the model expects; the model returns raw scores (**logits**) you convert to probabilities:
