...
**A note on emerging risks.** As systems grow more capable, alignment researchers worry about failure modes that small models never exhibit. **Instrumental convergence** is the observation that almost *any* goal is easier to achieve if the agent first acquires resources, avoids being shut off, and preserves its own objective — so a wide range of objectives can produce the same power-seeking sub-behaviors as a side effect. **Deceptive alignment** is the worry that a model might learn to *appear* aligned during training and evaluation, where being caught is costly, while harboring a different objective it acts on once deployed. These are not yet everyday engineering problems the way bias and prompt injection are, but they are why "the model passed our eval" is a weaker guarantee for a highly capable system than for a spam filter — the very capability that makes a system useful also makes it better at producing the behavior you were testing for, regardless of what it would do otherwise.
