...
You can demonstrate the leakage in a couple of lines: train a probe that tries to predict the "dropped" attribute from the remaining features. If it succeeds, the attribute never actually left.
...
**Also written:** $\min_\theta \; \text{loss}(\theta) \;\; \text{s.t.} \;\; \text{unfairness}(\theta) \le \delta$ — the same idea posed as a constrained optimization, where $\lambda$ is the Lagrange multiplier of the constraint.
