...
<style>
@keyframes c26-mf-user { 0%,100% { transform: rotate(-18deg);} 50% { transform: rotate(34deg);} }
@keyframes c26-mf-item { 0%,100% { transform: rotate(40deg);} 50% { transform: rotate(30deg);} }
@keyframes c26-mf-glow { 0%,100% { opacity:.25;} 50% { opacity:.9;} }
@media (prefers-reduced-motion: reduce){ .c26-mf-needle, .c26-mf-spark { animation: none !important; } }
</style>
<svg viewBox="0 0 360 170" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="11">
  <text x="180" y="16" text-anchor="middle" fill="currentColor">when the user dial and the item dial align → high predicted rating</text>
  <!-- user dial -->
  <circle cx="90" cy="95" r="42" fill="#6366f1" fill-opacity="0.12" stroke="currentColor"/>
  <line x1="90" y1="95" x2="90" y2="58" stroke="#6366f1" stroke-width="3" stroke-linecap="round"
        class="c26-mf-needle" style="transform-origin:90px 95px; animation:c26-mf-user 5s ease-in-out infinite"/>
  <text x="90" y="150" text-anchor="middle" fill="currentColor">user p_u</text>
  <!-- item dial -->
  <circle cx="270" cy="95" r="42" fill="#22c55e" fill-opacity="0.12" stroke="currentColor"/>
  <line x1="270" y1="95" x2="270" y2="58" stroke="#22c55e" stroke-width="3" stroke-linecap="round"
        class="c26-mf-needle" style="transform-origin:270px 95px; animation:c26-mf-item 5s ease-in-out infinite"/>
  <text x="270" y="150" text-anchor="middle" fill="currentColor">item q_i</text>
  <!-- alignment spark -->
  <text x="180" y="92" text-anchor="middle" font-size="22" fill="#f59e0b"
        class="c26-mf-spark" style="animation:c26-mf-glow 5s ease-in-out infinite">★</text>
  <text x="180" y="112" text-anchor="middle" fill="currentColor">p_u · q_i</text>
</svg>
...
We learn $p,q$ only from *observed* entries by minimizing squared error with regularization (this is the famous form from the Netflix Prize), typically via [SGD or alternating least squares](ch03.html):
...
The magic: two users who never rated the *same* film can still match, because both map to nearby latent vectors. MF generalizes where neighborhood methods see no overlap. Under the hood this is the same low-rank idea as the [SVD in Dimensionality Reduction](ch07.html) — we are finding a few hidden axes that explain most of the rating variance, just fit only on the cells we actually observed.
...
**Content-based filtering** ignores the crowd and looks at the *items themselves*. Each item gets a feature vector — genres, tags, the [TF-IDF](ch20.html) of its description, brand, price band — and each user gets a **profile** built from the features of items they liked. Recommend items whose feature vectors are closest to the profile.
...
<style>
@keyframes c26-conf-grow { 0% { transform: scaleY(0.05);} 60%,100% { transform: scaleY(1);} }
@media (prefers-reduced-motion: reduce){ .c26-conf-bar { animation: none !important; transform: scaleY(1); } }
</style>
<svg viewBox="0 0 340 150" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="10">
  <text x="170" y="14" text-anchor="middle" fill="currentColor">confidence c = 1 + α·count — more plays, harder pull</text>
  <line x1="40" y1="120" x2="320" y2="120" stroke="currentColor"/>
  <rect x="60"  y="112" width="40" height="8"   fill="#94a3b8" fill-opacity="0.7" stroke="currentColor"
        class="c26-conf-bar" style="transform-origin:60px 120px; animation:c26-conf-grow 3.5s ease-out infinite"/>
  <rect x="150" y="84"  width="40" height="36"  fill="#38bdf8" fill-opacity="0.6" stroke="currentColor"
        class="c26-conf-bar" style="transform-origin:150px 120px; animation:c26-conf-grow 3.5s ease-out infinite .3s"/>
  <rect x="240" y="34"  width="40" height="86"  fill="#22c55e" fill-opacity="0.6" stroke="currentColor"
        class="c26-conf-bar" style="transform-origin:240px 120px; animation:c26-conf-grow 3.5s ease-out infinite .6s"/>
  <text x="80"  y="135" text-anchor="middle" fill="currentColor">0 plays · c=1</text>
  <text x="170" y="135" text-anchor="middle" fill="currentColor">1 play · c=41</text>
  <text x="260" y="135" text-anchor="middle" fill="currentColor">3 plays · c=121</text>
</svg>
...
<style>
@keyframes c26-ltr-rise { 0%,15% { transform: translateY(72px);} 55%,100% { transform: translateY(0);} }
@keyframes c26-ltr-fade { 0%,15% { opacity:.35;} 55%,100% { opacity:1;} }
@media (prefers-reduced-motion: reduce){ .c26-ltr-hit { animation: none !important; transform: translateY(0); opacity:1;} }
</style>
<svg viewBox="0 0 300 150" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="11">
  <text x="150" y="14" text-anchor="middle" fill="currentColor">pairwise learning pulls the clicked item above the skipped ones</text>
  <rect x="100" y="28"  width="100" height="22" rx="4" fill="#94a3b8" fill-opacity="0.25" stroke="currentColor"/>
  <text x="150" y="43" text-anchor="middle" fill="currentColor">skipped</text>
  <rect x="100" y="56"  width="100" height="22" rx="4" fill="#94a3b8" fill-opacity="0.25" stroke="currentColor"/>
  <text x="150" y="71" text-anchor="middle" fill="currentColor">skipped</text>
  <g class="c26-ltr-hit" style="animation:c26-ltr-rise 4s ease-in-out infinite, c26-ltr-fade 4s ease-in-out infinite">
    <rect x="100" y="84" width="100" height="22" rx="4" fill="#22c55e" fill-opacity="0.45" stroke="currentColor"/>
    <text x="150" y="99" text-anchor="middle" fill="currentColor">clicked ★</text>
  </g>
</svg>
...
**Two-tower retrieval** is the workhorse of large-scale retrieval. A **user tower** maps user features to an embedding, an **item tower** maps item features to an [embedding](ch14.html) in the *same* space, and relevance is their dot product. The trick: item embeddings are computed offline for the whole catalog and stored in an **approximate nearest neighbor (ANN)** index, so at request time you embed the user once and fetch the nearest items in milliseconds — even from a billion-item catalog.
...
<style>
@keyframes c26-tt-flow { 0% { offset-distance:0%; opacity:0;} 12% { opacity:1;} 88% { opacity:1;} 100% { offset-distance:100%; opacity:0;} }
@media (prefers-reduced-motion: reduce){ .c26-tt-dot { animation: none !important; opacity:.7; } }
</style>
<svg viewBox="0 0 380 160" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="10">
  <text x="190" y="14" text-anchor="middle" fill="currentColor">two towers map into one shared space; nearest items win</text>
  <!-- user side -->
  <rect x="20" y="40" width="70" height="26" rx="5" fill="#6366f1" fill-opacity="0.25" stroke="currentColor"/>
  <text x="55" y="57" text-anchor="middle" fill="currentColor">user feats</text>
  <!-- item side -->
  <rect x="290" y="40" width="70" height="26" rx="5" fill="#22c55e" fill-opacity="0.25" stroke="currentColor"/>
  <text x="325" y="57" text-anchor="middle" fill="currentColor">item feats</text>
  <!-- shared space -->
  <ellipse cx="190" cy="105" rx="80" ry="34" fill="#38bdf8" fill-opacity="0.12" stroke="currentColor"/>
  <text x="190" y="108" text-anchor="middle" fill="currentColor">shared embedding space</text>
  <circle cx="178" cy="120" r="4" fill="#6366f1"/><circle cx="200" cy="118" r="4" fill="#22c55e"/>
  <!-- flow paths -->
  <path id="c26-tt-pu" d="M55 66 L178 116" fill="none" stroke="currentColor" stroke-opacity="0.3"/>
  <path id="c26-tt-pi" d="M325 66 L200 114" fill="none" stroke="currentColor" stroke-opacity="0.3"/>
  <circle r="3.5" fill="#6366f1" class="c26-tt-dot" style="offset-path:path('M55 66 L178 116'); animation:c26-tt-flow 3.2s linear infinite"/>
  <circle r="3.5" fill="#22c55e" class="c26-tt-dot" style="offset-path:path('M325 66 L200 114'); animation:c26-tt-flow 3.2s linear infinite .4s"/>
</svg>

**Sequence / session-based** models treat your recent activity as an ordered sequence and predict the next item — [RNNs](ch16.html) (GRU4Rec) or, now dominant, self-attention (SASRec, BERT4Rec, borrowing the machinery of [Attention & Transformers](ch17.html)). These shine when intent is *temporal*: what you want next depends on what you just did, even within an anonymous session with no long-term profile.
...
**Ranking** then applies an expensive, feature-rich model (wide-and-deep, [gradient-boosted trees](ch10.html), large neural nets) to just those few hundred, producing the precise order shown to the user. Optimized for *precision at the top*. Often a third **re-ranking** stage adds business rules and diversity.
...
::: {.callout-note appearance="simple"}
**↪ The thread continues → [Chapter 27 · 🚨 Anomaly & Fraud Detection](ch27.html)**
