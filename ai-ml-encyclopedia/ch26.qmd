...
Neighborhood methods compare raw rows or columns. **Matrix factorization (MF)** instead compresses the matrix. It assumes each user and each item lives in the same small **latent factor** space of dimension $k$ (think hidden axes like "action ↔ drama," "mainstream ↔ niche"). User $u$ becomes a vector $p_u\in\mathbb{R}^k$, item $i$ a vector $q_i\in\mathbb{R}^k$, and the predicted rating is their dot product:
...
- **Feature combination / model-based** — feed both collaborative signals (latent factors) and content features into one model (the wide-and-deep and two-tower architectures in §26.5 are exactly this).
...
## 26.6 — Candidate generation vs ranking
...
## 26.7 — Evaluation: precision@k, recall@k, NDCG, MAP
...
## 26.8 — A worked similarity example
