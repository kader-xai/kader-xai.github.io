...
**The alignment gap.** One subtlety worth naming early: even in a well-trained shared space, image vectors and text vectors do not perfectly overlap. They tend to occupy two slightly separated *cones* — image embeddings cluster among themselves, text embeddings among themselves, with a narrow gap between the clusters. This **modality gap** is partly a consequence of random initialization and the temperature in contrastive training, and it is usually harmless (cosine ranking still works fine within a modality-to-modality comparison), but it explains why naive tricks like averaging an image vector with a text vector can land you in an empty no-man's-land between the cones.
