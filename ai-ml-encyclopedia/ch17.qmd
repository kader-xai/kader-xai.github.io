...
Editor flags resolved: removed the preamble; unified the gap convention to $(n-m)$ throughout §17.90; relabeled the RoPE SVG as one vector rotated by its own absolute angle $m\theta$ (not two queries, not the relative term) and seated both arrowheads on their line ends; renamed the ALiBi slope to $s$ to avoid colliding with query position $m$; trimmed the RoPE-extension paragraph to one sentence naming PI / NTK / YaRN; and hedged the NoPE claim to "can, in some settings."

Not delivered here (and not in scope of these two sections): the MoE routing diagram, the modern-decoder-block `{mermaid}`, and the model-comparison table the ch17-rope checklist also lists. These two sections (§17.90, §17.91) contain no MoE / modern-component content, so if the checklist requires those visuals they must live in sibling sections — confirm they are delivered elsewhere, or they are missing.

## 17.7 — Beyond Quadratic Attention: Efficient Attention and State-Space Models
...
## 17.8 — Linear and Efficient Attention: Getting Rid of the Square
...
## 17.9 — State-Space Models: S4 and Mamba
