...
Many of the models above — exponential smoothing, ARIMA, structural time series — are secretly the *same* object wearing different clothes: a **state-space model**. The intuition is a tracking radar. There is a hidden "true state" of the world you cannot see directly (an aircraft's real position and velocity), it evolves by some rule, and you only get noisy measurements. A **state-space model** writes this as two equations: a *transition* equation for how the hidden state moves, and an *observation* equation for how measurements relate to it.
