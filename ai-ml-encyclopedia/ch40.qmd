...
The trouble is that the grid and sequence models from earlier chapters ([CNNs](ch15.html), [RNNs](ch16.html)/[Transformers](ch17.html)) lean on a regularity graphs do not have:
...
Before end-to-end graph networks took over, a beautifully simple idea dominated: **treat random walks on the graph like sentences, and apply word2vec.** If you stand on a node and wander to random neighbors, the sequence of nodes you visit is like a sentence whose "words" are nodes. Nodes that co-occur in many walks are "used in similar contexts," so — exactly as in the [skip-gram model](ch20.html) — they should get similar embeddings.
...
The softmax denominator over all nodes is too expensive, so in practice it is approximated with **negative sampling** ([Chapter 20](ch20.html)): push $z_u^\top z_v$ up for real walk-neighbors, push it down for a handful of random "negative" nodes.
...
<style>
@keyframes c40-walk { 0%{offset-distance:0%} 100%{offset-distance:100%} }
.c40-walker { offset-path: path('M 40 70 L 140 40 L 240 80 L 340 45 L 410 75'); animation: c40-walk 5s linear infinite; }
@media (prefers-reduced-motion: reduce){ .c40-walker{ animation: none } }
</style>
<svg viewBox="0 0 460 130" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="12">
  <polyline points="40,70 140,40 240,80 340,45 410,75" fill="none" stroke="currentColor" stroke-width="1.5" stroke-dasharray="4 4" opacity="0.5"/>
  <g fill="#6366f1">
    <circle cx="40" cy="70" r="9"/><circle cx="140" cy="40" r="9"/><circle cx="240" cy="80" r="9"/><circle cx="340" cy="45" r="9"/><circle cx="410" cy="75" r="9"/>
  </g>
  <g fill="currentColor" text-anchor="middle">
    <text x="40" y="74" fill="#fff">2</text><text x="140" y="44" fill="#fff">0</text><text x="240" y="84" fill="#fff">2</text><text x="340" y="49" fill="#fff">3</text><text x="410" y="79" fill="#fff">4</text>
  </g>
  <circle class="c40-walker" r="5" fill="#f59e0b"/>
  <text x="230" y="118" text-anchor="middle" fill="currentColor">walk [2, 0, 2, 3, 4]  →  a "sentence" of nodes for skip-gram</text>
</svg>
...
<style>
@keyframes c40-msg { 0%{ r:2; opacity:0 } 20%{ opacity:1 } 100%{ r:2; opacity:0; cx:230px; cy:75px } }
@keyframes c40-pulse { 0%,60%{ opacity:.4 } 80%{ opacity:1 } 100%{ opacity:.4 } }
.c40-m1{ animation: c40-msg 3s ease-in 0s infinite; }
.c40-m2{ animation: c40-msg 3s ease-in 0s infinite; }
.c40-m3{ animation: c40-msg 3s ease-in 0s infinite; }
.c40-center{ animation: c40-pulse 3s ease-in-out 0s infinite; }
@media (prefers-reduced-motion: reduce){ .c40-m1,.c40-m2,.c40-m3,.c40-center{ animation: none } }
</style>
<svg viewBox="0 0 460 150" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="12">
  <g stroke="currentColor" stroke-width="1.5" opacity="0.5">
    <line x1="90" y1="40" x2="230" y2="75"/><line x1="80" y1="115" x2="230" y2="75"/><line x1="370" y1="55" x2="230" y2="75"/>
  </g>
  <g fill="#38bdf8"><circle cx="90" cy="40" r="11"/><circle cx="80" cy="115" r="11"/><circle cx="370" cy="55" r="11"/></g>
  <circle class="c40-center" cx="230" cy="75" r="15" fill="#22c55e"/>
  <text x="230" y="79" text-anchor="middle" fill="#fff">v</text>
  <text x="90" y="44" text-anchor="middle" fill="#fff" font-size="10">u</text>
  <text x="80" y="119" text-anchor="middle" fill="#fff" font-size="10">w</text>
  <text x="370" y="59" text-anchor="middle" fill="#fff" font-size="10">x</text>
  <circle class="c40-m1" cx="90" cy="40" fill="#f59e0b"/>
  <circle class="c40-m2" cx="80" cy="115" fill="#f59e0b"/>
  <circle class="c40-m3" cx="370" cy="55" fill="#f59e0b"/>
  <text x="230" y="135" text-anchor="middle" fill="currentColor">neighbors send → v aggregates → v updates (one layer)</text>
</svg>
...
The bars are the attention weights $\alpha$ above: the strongest neighbor glows brightest, and the widths sum to one whole row.

<style>
@keyframes c40-attn { 0%,100%{ opacity:.55 } 50%{ opacity:1 } }
.c40-bar{ animation: c40-attn 3.5s ease-in-out infinite; }
@media (prefers-reduced-motion: reduce){ .c40-bar{ animation: none } }
</style>
<svg viewBox="0 0 460 120" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="12">
  <text x="30" y="40" fill="currentColor">v</text>
  <circle cx="30" cy="60" r="12" fill="#22c55e"/><text x="30" y="64" text-anchor="middle" fill="#fff">v</text>
  <rect class="c40-bar" x="90" y="38" width="263" height="22" rx="4" fill="#a855f7"/>
  <rect class="c40-bar" x="90" y="64" width="97"  height="22" rx="4" fill="#a855f7" style="animation-delay:.4s"/>
  <rect class="c40-bar" x="90" y="90" width="40"  height="22" rx="4" fill="#a855f7" style="animation-delay:.8s"/>
  <g fill="currentColor" font-size="11">
    <text x="360" y="54">u₁ · α=0.659</text>
    <text x="194" y="80">u₂ · α=0.242</text>
    <text x="137" y="106">u₃ · α=0.099</text>
  </g>
</svg>
...
- **Recommenders** — the user–item interaction graph is enormous and sparse; GNNs (e.g. PinSAGE-style, building on [recommender systems](ch26.html)) power web-scale recommendation by aggregating over the bipartite graph.
...
- **Fraud & security** — [fraud rings](ch27.html) show up as suspicious subgraph structure that node-feature models miss entirely.
...
- **[Chapter 20 (Natural Language Processing)](ch20.html)** — skip-gram and negative sampling, the engine behind DeepWalk/node2vec.
- **[Chapter 17 (Attention & Transformers)](ch17.html)** — the attention mechanism GAT imports onto edges, and the global attention graph transformers (§40.12) generalize; Transformers are essentially GNNs on a fully connected graph.
- **[Chapter 15 (Convolutional Neural Networks)](ch15.html)** — the grid-convolution intuition GCN generalizes to irregular neighborhoods.
- **[Chapter 10 (Ensemble Methods)](ch10.html)** — the strong tabular baseline (gradient-boosted trees) relational deep learning competes with.
- **[Chapter 26 (Recommender Systems)](ch26.html)** — the user–item graphs where web-scale GNNs (PinSAGE) shine.
- **[Chapter 39 (Frontier & Emerging Directions)](ch39.html)** — higher-order and geometric deep learning that push past the 1-WL expressiveness ceiling.

------------------------------------------------------------------------

::: {.callout-note appearance="simple"}
**↪ The thread continues → [Chapter 41 · 🤖 Robotics & Autonomy](ch41.html)**
