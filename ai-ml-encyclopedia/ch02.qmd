...
**Forward vs reverse mode.** Autodiff comes in two flavors, and the choice is about *shape*. Picture the chain rule's product of derivatives: you can multiply it left-to-right or right-to-left, and which is cheaper depends on whether the function fans *out* (few inputs, many outputs) or *in* (many inputs, one output).

- **Forward mode** carries derivatives *forward* alongside the values, input by input. It is cheap when there are few inputs and many outputs ($n \ll m$).
- **Reverse mode** does one forward pass to record the graph, then sweeps *backward* from the output. It is cheap when there are many inputs and few outputs ($n \gg m$) — which is *exactly* the deep-learning case: millions of parameters, one scalar loss. **Reverse mode is backpropagation.**
