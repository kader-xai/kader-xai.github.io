...
Convolutional Neural Networks (CNNs) are the family of deep models built to understand data laid out on a grid — above all, images. They replace the dense "everything connects to everything" wiring of ordinary [neural networks](ch14.html) with small sliding filters that look for local patterns, which makes them dramatically more efficient and far better at vision. CNNs sit squarely inside Deep Learning and are the workhorse behind most [computer vision](ch19.html) systems.
...
**AlexNet** (2012) was the same recipe scaled up and run on GPUs, and it crushed the ImageNet competition, kicking off the deep-learning boom. Its contributions were practical: **ReLU** [activations](ch14.html) (faster training than tanh), **dropout** for regularisation, and data augmentation.
...
**Residual connections.** Naively stacking more layers made networks *worse* — gradients vanished and very deep nets were hard to [optimise](ch03.html). ResNet adds a **skip connection** that lets the input bypass a block and be added to its output:
...
- [**Neural Networks (Core)**](ch14.html) — activations, backprop, dropout, and the dense layers CNNs build on.
- [**Computer Vision**](ch19.html) — detection, segmentation, and the tasks CNNs power.
- [**Generative Models**](ch18.html) — convolutional GANs and autoencoders that run convolutions in reverse.
- [**Attention & Transformers**](ch17.html) — Vision Transformers, the modern alternative to convolutional backbones.
- [**Optimization**](ch03.html) — SGD, learning-rate schedules, and the gradient flow residual connections protect.
- [**MLOps & Deployment**](ch29.html) — serving and compressing trained CNN backbones in production.

------------------------------------------------------------------------

::: {.callout-note appearance="simple"}
**↪ The thread continues → [Chapter 16 · 🔁 Recurrent & Sequence Models](ch16.html)**
