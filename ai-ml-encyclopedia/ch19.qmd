...
CLIP embeddings underpin much of modern multimodal AI (Chapter on Multimodal owns the full story) and are the text-grounding behind open-vocabulary detectors and segmenters.
...
::: {.callout-warning}
Adversarial examples are not a lab curiosity. Printed stickers on a stop sign, patterned glasses that fool face recognition, and subtle audio noise have all been demonstrated in the physical world. Robustness is a *security* property, not just an accuracy one — and high clean accuracy tells you nothing about it.
:::

### Explainability: seeing what the model looked at

When a model says "tumor" or "stop sign," you often need to know *which pixels* drove that call — for trust, for debugging, and for catching the classic failure where a model "detects boats" by actually detecting water. **Saliency** and **class-activation** methods produce a heatmap over the image showing where the evidence for a class came from.
