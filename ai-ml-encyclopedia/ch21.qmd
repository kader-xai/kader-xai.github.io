...
<style>
@keyframes c21-slide { 0% { transform: translateX(0); } 100% { transform: translateX(150px); } }
@keyframes c21-fillcol { 0%,100% { opacity: 0.25; } 50% { opacity: 0.85; } }
@media (prefers-reduced-motion: reduce) {
  .c21-window, .c21-col { animation: none !important; }
}
</style>
<svg viewBox="0 0 560 200" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="12">
  <text x="100" y="18" text-anchor="middle" font-weight="bold" fill="currentColor">Waveform (time)</text>
  <text x="420" y="18" text-anchor="middle" font-weight="bold" fill="currentColor">Spectrogram (time × freq)</text>
  <!-- waveform -->
  <polyline points="20,90 30,60 40,120 50,55 60,125 70,70 80,110 90,50 100,130 110,65 120,105 130,58 140,122 150,72 160,108 170,52 180,128 190,68 200,100"
    fill="none" stroke="#38bdf8" stroke-width="1.6"/>
  <!-- sliding window -->
  <rect class="c21-window" x="20" y="40" width="40" height="100" fill="#f59e0b" opacity="0.25" stroke="#f59e0b"
    style="animation: c21-slide 5s ease-in-out infinite alternate"/>
  <!-- arrow -->
  <line x1="220" y1="90" x2="280" y2="90" stroke="currentColor" stroke-width="1.5" marker-end="url(#c21a)"/>
  <text x="250" y="80" text-anchor="middle" fill="currentColor" font-size="10">STFT</text>
  <!-- spectrogram grid -->
  <g>
    <rect class="c21-col" x="300" y="40" width="18" height="100" fill="#6366f1" style="animation: c21-fillcol 5s ease-in-out infinite; animation-delay:0s"/>
    <rect class="c21-col" x="320" y="40" width="18" height="100" fill="#22c55e" style="animation: c21-fillcol 5s ease-in-out infinite; animation-delay:0.6s"/>
    <rect class="c21-col" x="340" y="40" width="18" height="100" fill="#a855f7" style="animation: c21-fillcol 5s ease-in-out infinite; animation-delay:1.2s"/>
    <rect class="c21-col" x="360" y="40" width="18" height="100" fill="#ec4899" style="animation: c21-fillcol 5s ease-in-out infinite; animation-delay:1.8s"/>
    <rect class="c21-col" x="380" y="40" width="18" height="100" fill="#38bdf8" style="animation: c21-fillcol 5s ease-in-out infinite; animation-delay:2.4s"/>
    <rect class="c21-col" x="400" y="40" width="18" height="100" fill="#6366f1" style="animation: c21-fillcol 5s ease-in-out infinite; animation-delay:3.0s"/>
    <rect class="c21-col" x="420" y="40" width="18" height="100" fill="#22c55e" style="animation: c21-fillcol 5s ease-in-out infinite; animation-delay:3.6s"/>
    <rect class="c21-col" x="440" y="40" width="18" height="100" fill="#a855f7" style="animation: c21-fillcol 5s ease-in-out infinite; animation-delay:4.2s"/>
  </g>
  <text x="378" y="158" text-anchor="middle" fill="currentColor" font-size="10">each window → one column of energies</text>
  <defs><marker id="c21a" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="currentColor"/></marker></defs>
</svg>
...
**The classical pipeline (pre-2015).** Old systems factored the problem with [Bayes' rule](ch04.html). To find the most likely word sequence $W$ given acoustics $X$:
...
**Attention / sequence-to-sequence.** A second end-to-end family uses an **encoder–decoder with attention** (see [Attention & Transformers](ch17.html)): the encoder turns audio into a sequence of vectors, and the decoder generates text one token at a time, *attending* to the relevant audio frames at each step. This learns a soft alignment jointly with the language, often beating CTC on fluency — at the cost of needing the whole utterance before decoding, which is bad for streaming.
...
<style>
@keyframes c21-ar-pulse { 0%,100% { r: 4; opacity: 0.4; } 50% { r: 7; opacity: 1; } }
@keyframes c21-ar-march { 0% { cx: 70; } 100% { cx: 430; } }
@media (prefers-reduced-motion: reduce) {
  .c21-ar-head { animation: c21-ar-pulse 3s ease-in-out infinite !important; }
}
</style>
<svg viewBox="0 0 480 130" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="12">
  <text x="240" y="18" text-anchor="middle" font-weight="bold" fill="currentColor">Autoregressive generation: predict x_t from its past</text>
  <line x1="40" y1="80" x2="450" y2="80" stroke="currentColor" stroke-width="1" opacity="0.4"/>
  <g fill="#6366f1">
    <circle cx="70" cy="80" r="4"/><circle cx="110" cy="80" r="4"/><circle cx="150" cy="80" r="4"/>
    <circle cx="190" cy="80" r="4"/><circle cx="230" cy="80" r="4"/><circle cx="270" cy="80" r="4"/>
    <circle cx="310" cy="80" r="4"/><circle cx="350" cy="80" r="4"/><circle cx="390" cy="80" r="4"/>
  </g>
  <!-- moving prediction head -->
  <circle class="c21-ar-head" cx="70" cy="80" r="6" fill="#ec4899"
    style="animation: c21-ar-march 6s steps(9) infinite, c21-ar-pulse 0.66s ease-in-out infinite"/>
  <text x="240" y="112" text-anchor="middle" fill="currentColor" font-size="10">each new sample depends on the whole history to its left → slow but high fidelity</text>
</svg>
...
The catch with WaveNet was speed: predicting 24,000 samples per second one at a time is painfully slow. Later vocoders — parallel WaveNet, **HiFi-GAN** (a [GAN](ch18.html)-based vocoder) — generate all samples at once, hitting real-time or faster while keeping quality high. Today, end-to-end systems such as VITS fold both stages into a single network.
...
- **Speaker diarization**: "Who spoke when?" Given a meeting recording with unknown speakers, segment it and label each segment — *Speaker A, Speaker B…* The classic approach slices audio into short windows, embeds each, then [**clusters**](ch11.html) the embeddings; each cluster becomes one speaker.
...
<svg viewBox="0 0 420 180" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="12">
  <text x="210" y="18" text-anchor="middle" font-weight="bold" fill="currentColor">Speaker embeddings cluster by voice</text>
  <!-- cluster A -->
  <circle cx="120" cy="100" r="50" fill="#6366f1" opacity="0.12" stroke="#6366f1" stroke-dasharray="4 3"/>
  <g fill="#6366f1">
    <circle cx="100" cy="90" r="5"/><circle cx="125" cy="80" r="5"/><circle cx="115" cy="110" r="5"/>
    <circle cx="140" cy="105" r="5"/><circle cx="105" cy="120" r="5"/>
  </g>
  <text x="120" y="165" text-anchor="middle" fill="currentColor" font-size="11">Speaker A</text>
  <!-- cluster B -->
  <circle cx="300" cy="90" r="50" fill="#22c55e" opacity="0.12" stroke="#22c55e" stroke-dasharray="4 3"/>
  <g fill="#22c55e">
    <circle cx="290" cy="80" r="5"/><circle cx="315" cy="75" r="5"/><circle cx="305" cy="100" r="5"/>
    <circle cx="325" cy="95" r="5"/><circle cx="285" cy="105" r="5"/>
  </g>
  <text x="300" y="155" text-anchor="middle" fill="currentColor" font-size="11">Speaker B</text>
</svg>
...
<style>
@keyframes c21-kws-listen { 0%,90%,100% { opacity: 0.3; } 45% { opacity: 0.7; } }
@keyframes c21-kws-fire { 0%,40% { opacity: 0.15; transform: scale(0.9); } 60%,100% { opacity: 0.9; transform: scale(1); } }
@keyframes c21-kws-bars { 0%,100% { height: 8px; y: 76px; } 50% { height: 28px; y: 56px; } }
@media (prefers-reduced-motion: reduce) {
  .c21-kws-pulse, .c21-kws-bar { animation: none !important; }
}
</style>
<svg viewBox="0 0 460 150" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="12">
  <text x="230" y="18" text-anchor="middle" font-weight="bold" fill="currentColor">Tiny gatekeeper → wakes the heavy model</text>
  <!-- mic / listening bars -->
  <g>
    <rect class="c21-kws-bar" x="40" y="76" width="8" height="8" fill="#38bdf8" style="animation: c21-kws-bars 1.4s ease-in-out infinite; animation-delay:0s"/>
    <rect class="c21-kws-bar" x="52" y="76" width="8" height="8" fill="#38bdf8" style="animation: c21-kws-bars 1.4s ease-in-out infinite; animation-delay:0.2s"/>
    <rect class="c21-kws-bar" x="64" y="76" width="8" height="8" fill="#38bdf8" style="animation: c21-kws-bars 1.4s ease-in-out infinite; animation-delay:0.4s"/>
  </g>
  <text x="56" y="110" text-anchor="middle" fill="currentColor" font-size="10">always-on mic</text>
  <!-- tiny KWS box -->
  <rect x="110" y="55" width="90" height="44" rx="6" fill="#22c55e" opacity="0.3" stroke="currentColor"/>
  <text x="155" y="74" text-anchor="middle" fill="currentColor" font-size="10">tiny KWS</text>
  <text x="155" y="89" text-anchor="middle" fill="currentColor" font-size="9">low power</text>
  <!-- arrow firing -->
  <line x1="205" y1="77" x2="255" y2="77" stroke="#f59e0b" stroke-width="2" marker-end="url(#c21k)"/>
  <text x="230" y="68" text-anchor="middle" fill="currentColor" font-size="9">"Hey…"</text>
  <!-- heavy model, pulses on -->
  <g class="c21-kws-pulse" style="animation: c21-kws-fire 4s ease-in-out infinite; transform-origin: 330px 77px">
    <rect x="262" y="48" width="140" height="58" rx="8" fill="#a855f7" opacity="0.35" stroke="currentColor"/>
    <text x="332" y="72" text-anchor="middle" fill="currentColor" font-size="10">full ASR + NLU</text>
    <text x="332" y="88" text-anchor="middle" fill="currentColor" font-size="9">(cloud, ~1% of the time)</text>
  </g>
  <defs><marker id="c21k" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#f59e0b"/></marker></defs>
</svg>
...
This is the audio version of what BERT did for [text](ch20.html), and it is why a model like **wav2vec 2.0** can reach strong WER with only ten minutes of labeled speech after pretraining on tens of thousands of unlabeled hours.
...
::: {.callout-note appearance="simple"}
**↪ The thread continues → [Chapter 22 · ⏳ Time Series & Forecasting](ch22.html)**
