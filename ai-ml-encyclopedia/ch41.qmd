...
- Plan in **configuration space**, where the robot is a point and obstacles are inflated by its footprint. Wheeled robots are **nonholonomic**: fewer controls than DOF, so not every path is achievable.
- **Kinematics** maps commands to motion; **control** (PID, feedback linearization, flatness, and LQR from Ch 25) drives tracking error to zero despite disturbances. Watch for integral windup.
...
- **SLAM** estimates trajectory and map together; **loop closure** bounds otherwise-unbounded odometry drift, and the back-end is a graph optimization.
...
- **Grasping** hinges on **force closure** within friction cones; manipulation spans **prehensile** (grasping) and **non-prehensile** (pushing, sliding) contact.
