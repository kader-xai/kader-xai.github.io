...
**Autoregressive generation** is the root of the difference: the model produces token 1, appends it to the input, produces token 2 from the longer sequence, and so on. The cost of a 500-token answer is roughly 500 sequential forward passes. The single most important optimization is the **KV cache** — the attention keys and values computed for tokens already generated are stored and reused, so each new token is *one* cheap step instead of re-reading the whole sequence. The serving cost then splits cleanly into two phases:
