...
- **Adversarial examples** — inputs perturbed just enough to flip the prediction (a few pixels change a "stop sign" into "speed limit" to the model). Mitigation: input sanitization, adversarial training, anomaly detection on inputs.
...
- **Bias & fairness checks as a pipeline gate** — fairness is not a one-time audit but a *test in the CI pipeline* that blocks promotion if a subgroup metric regresses, exactly like a unit test (see *Ethics & Responsible AI*).
