...
```{=html}
<style>
@keyframes c37-climb { 0%,100%{transform:translateY(0)} 33%{transform:translateY(-44px)} 66%{transform:translateY(-88px)} }
@keyframes c37-glow  { 0%,100%{opacity:.35} 50%{opacity:1} }
@media (prefers-reduced-motion: reduce){ .c37-walker{animation:none!important} }
</style>
<svg viewBox="0 0 420 200" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="12" role="img" aria-label="Animated ladder of causation: a figure climbing seeing, doing, imagining">
  <!-- three rungs -->
  <rect x="120" y="150" width="240" height="20" rx="4" fill="#38bdf8" opacity="0.25"/>
  <rect x="120" y="106" width="240" height="20" rx="4" fill="#f59e0b" opacity="0.25"/>
  <rect x="120" y="62"  width="240" height="20" rx="4" fill="#ec4899" opacity="0.25"/>
  <text x="132" y="164" fill="currentColor">① Seeing &#160; P(Y|X)</text>
  <text x="132" y="120" fill="currentColor">② Doing &#160; P(Y|do X)</text>
  <text x="132" y="76"  fill="currentColor">③ Imagining &#160; counterfactual</text>
  <!-- climbing dot -->
  <g class="c37-walker" style="animation:c37-climb 6s ease-in-out infinite">
    <circle cx="95" cy="160" r="9" fill="#a855f7"/>
    <circle cx="95" cy="160" r="14" fill="none" stroke="#a855f7" stroke-width="2" style="animation:c37-glow 2s ease-in-out infinite"/>
  </g>
</svg>
```
...
A standard [supervised model](ch05.html) learns $P(Y \mid X)$: the distribution of the target given the features *as they naturally occur*. That is exactly the right object when you only want to **observe and predict** — you see someone's features and guess their outcome. It is the wrong object the moment you want to **act**, because acting changes the world in a way that the observed correlation may not survive.
...
```{=html}
<style>
@keyframes c37-seesee { 0%,45%{opacity:1} 55%,100%{opacity:.12} }
@keyframes c37-dosee  { 0%,45%{opacity:.12} 55%,100%{opacity:1} }
@keyframes c37-scissor{ 0%,40%{opacity:0;transform:translateX(-8px)} 55%,100%{opacity:1;transform:translateX(0)} }
@media (prefers-reduced-motion: reduce){ .c37-w1,.c37-w2,.c37-cut{animation:none!important} }
</style>
<svg viewBox="0 0 460 170" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="11" role="img" aria-label="Observing keeps the heat to ice-cream arrow; doing cuts it">
  <text x="230" y="16" text-anchor="middle" fill="currentColor" font-weight="bold">same graph, two questions</text>
  <!-- heat node -->
  <circle cx="230" cy="55" r="20" fill="#f59e0b" opacity="0.7"/>
  <text x="230" y="59" text-anchor="middle" fill="currentColor">heat</text>
  <!-- ice cream + drowning -->
  <rect x="80"  y="110" width="90" height="30" rx="6" fill="#38bdf8" opacity="0.3"/>
  <rect x="290" y="110" width="90" height="30" rx="6" fill="#22c55e" opacity="0.3"/>
  <text x="125" y="129" text-anchor="middle" fill="currentColor">ice cream</text>
  <text x="335" y="129" text-anchor="middle" fill="currentColor">drownings</text>
  <!-- heat -> drownings (always there) -->
  <line x1="245" y1="70" x2="320" y2="108" stroke="#22c55e" stroke-width="2.5"/>
  <!-- heat -> ice cream: present when OBSERVING, cut when DOING -->
  <line class="c37-w1" x1="215" y1="70" x2="140" y2="108" stroke="#f59e0b" stroke-width="2.5" style="animation:c37-seesee 6s ease-in-out infinite"/>
  <g class="c37-cut" style="animation:c37-scissor 6s ease-in-out infinite">
    <line x1="215" y1="70" x2="140" y2="108" stroke="#ec4899" stroke-width="2.5" stroke-dasharray="3 4"/>
    <text x="172" y="84" fill="#ec4899" font-size="14">✂</text>
  </g>
  <!-- caption switches -->
  <text class="c37-w1" x="230" y="162" text-anchor="middle" fill="#38bdf8" style="animation:c37-seesee 6s ease-in-out infinite">SEE: P(Y|X) — heat still feeds ice cream</text>
  <text class="c37-w2" x="230" y="162" text-anchor="middle" fill="#ec4899" style="animation:c37-dosee 6s ease-in-out infinite">DO: P(Y|do X) — you cut heat→ice cream</text>
</svg>
```
...
```{=html}
<style>
@keyframes c37-tug { 0%,100%{transform:translateY(0)} 50%{transform:translateY(6px)} }
@media (prefers-reduced-motion: reduce){ .c37-tug{animation:none!important} }
</style>
<svg viewBox="0 0 420 170" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="12" role="img" aria-label="A hidden confounder pulls strings on both treatment and outcome">
  <!-- puppeteer (hidden cause) -->
  <circle cx="210" cy="28" r="16" fill="#a855f7" opacity="0.7"/>
  <text x="210" y="14" text-anchor="middle" fill="currentColor">hidden cause C</text>
  <!-- strings, animated tug -->
  <g class="c37-tug" style="animation:c37-tug 3s ease-in-out infinite">
    <line x1="200" y1="40" x2="100" y2="110" stroke="#a855f7" stroke-width="2" stroke-dasharray="4 4"/>
    <line x1="220" y1="40" x2="320" y2="110" stroke="#a855f7" stroke-width="2" stroke-dasharray="4 4"/>
    <rect x="70"  y="112" width="60" height="26" rx="5" fill="#f59e0b" opacity="0.3"/>
    <rect x="290" y="112" width="60" height="26" rx="5" fill="#22c55e" opacity="0.3"/>
    <text x="100" y="129" text-anchor="middle" fill="currentColor">treatment</text>
    <text x="320" y="129" text-anchor="middle" fill="currentColor">outcome</text>
  </g>
  <!-- the spurious link we "see" -->
  <line x1="130" y1="150" x2="290" y2="150" stroke="#ec4899" stroke-width="2" stroke-dasharray="2 5"/>
  <text x="210" y="166" text-anchor="middle" fill="#ec4899">the fake link we observe</text>
</svg>
```
...
<svg viewBox="0 0 460 200" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="12">
  <line x1="40" y1="170" x2="440" y2="170" stroke="currentColor" opacity="0.4"/>
  <line x1="40" y1="20" x2="40" y2="170" stroke="currentColor" opacity="0.4"/>
  <text x="14" y="100" transform="rotate(-90 14,100)" fill="currentColor">success rate</text>
  <circle cx="120" cy="35" r="5" fill="#22c55e"/>
  <circle cx="360" cy="60" r="5" fill="#22c55e"/>
  <line x1="120" y1="35" x2="360" y2="60" stroke="#22c55e" stroke-width="2"/>
  <text x="370" y="60" fill="#22c55e">A</text>
  <circle cx="120" cy="48" r="5" fill="#f59e0b"/>
  <circle cx="360" cy="70" r="5" fill="#f59e0b"/>
  <line x1="120" y1="48" x2="360" y2="70" stroke="#f59e0b" stroke-width="2"/>
  <text x="370" y="74" fill="#f59e0b">B</text>
  <text x="95" y="190" fill="currentColor">small stones</text>
  <text x="325" y="190" fill="currentColor">large stones</text>
  <text x="120" y="150" fill="currentColor" opacity="0.75">A is above B in BOTH groups — yet B's easy-case mix wins overall</text>
</svg>
...
```{=html}
<style>
@keyframes c37-take  { 0%,100%{opacity:1} } /* taken road: solid */
@keyframes c37-ghost { 0%,100%{opacity:.25} 50%{opacity:.55} }
@media (prefers-reduced-motion: reduce){ .c37-ghost{animation:none!important} }
</style>
<svg viewBox="0 0 440 170" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="11" role="img" aria-label="A unit takes one road; the other road is the unobserved counterfactual">
  <!-- the unit -->
  <circle cx="60" cy="85" r="12" fill="#a855f7"/>
  <text x="60" y="118" text-anchor="middle" fill="currentColor">patient i</text>
  <!-- road taken: treated -->
  <line x1="72" y1="80" x2="300" y2="40" stroke="#22c55e" stroke-width="3"/>
  <circle cx="312" cy="38" r="11" fill="#22c55e" opacity="0.85"/>
  <text x="360" y="42" fill="currentColor">Y(1) observed ✓</text>
  <!-- road not taken: ghosted counterfactual -->
  <line class="c37-ghost" x1="72" y1="92" x2="300" y2="132" stroke="#ec4899" stroke-width="3" stroke-dasharray="5 6" style="animation:c37-ghost 4s ease-in-out infinite"/>
  <circle class="c37-ghost" cx="312" cy="134" r="11" fill="none" stroke="#ec4899" stroke-width="2" stroke-dasharray="3 3" style="animation:c37-ghost 4s ease-in-out infinite"/>
  <text class="c37-ghost" x="360" y="138" fill="#ec4899" style="animation:c37-ghost 4s ease-in-out infinite">Y(0) counterfactual ✕</text>
  <text x="170" y="160" text-anchor="middle" fill="currentColor">effect τ = Y(1) − Y(0): one half always missing</text>
</svg>
```
...
```{=html}
<style>
@keyframes c37-flip { 0%{transform:rotateX(0)} 50%{transform:rotateX(180deg)} 100%{transform:rotateX(360deg)} }
@media (prefers-reduced-motion: reduce){ .c37-coin{animation:none!important} }
</style>
<svg viewBox="0 0 420 150" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="12" role="img" aria-label="A coin flip sorts mixed units evenly into two balanced arms">
  <!-- mixed pool -->
  <text x="60" y="20" text-anchor="middle" fill="currentColor">mixed units</text>
  <circle cx="40" cy="50" r="6" fill="#f59e0b"/><circle cx="60" cy="42" r="6" fill="#22c55e"/>
  <circle cx="78" cy="58" r="6" fill="#38bdf8"/><circle cx="52" cy="68" r="6" fill="#ec4899"/>
  <!-- spinning coin -->
  <g class="c37-coin" style="animation:c37-flip 2.4s linear infinite;transform-origin:210px 75px">
    <circle cx="210" cy="75" r="20" fill="#6366f1" opacity="0.85"/>
    <text x="210" y="80" text-anchor="middle" fill="currentColor" font-weight="bold">½</text>
  </g>
  <!-- two balanced arms -->
  <text x="360" y="20" text-anchor="middle" fill="currentColor">balanced arms</text>
  <rect x="300" y="32" width="110" height="40" rx="5" fill="#6366f1" opacity="0.12"/>
  <rect x="300" y="80" width="110" height="40" rx="5" fill="#6366f1" opacity="0.12"/>
  <circle cx="320" cy="52" r="5" fill="#f59e0b"/><circle cx="345" cy="52" r="5" fill="#22c55e"/><circle cx="370" cy="52" r="5" fill="#38bdf8"/>
  <circle cx="320" cy="100" r="5" fill="#ec4899"/><circle cx="345" cy="100" r="5" fill="#f59e0b"/><circle cx="370" cy="100" r="5" fill="#22c55e"/>
  <text x="285" y="56" text-anchor="end" fill="currentColor">A</text>
  <text x="285" y="104" text-anchor="end" fill="currentColor">B</text>
</svg>
```

**Worked example.** Show variant B of a checkout page to a random 50% of visitors. Conversions: control 1,000/20,000 = 5.00%, variant 1,120/20,000 = 5.60%. Lift = 0.60 percentage points. Because assignment was random, no confounder can explain the gap — heavy buyers, mobile users, time-of-day all split evenly across arms in expectation. The only remaining question is whether 0.6 points is signal or noise, which a [significance test](ch04.html) answers:
...
Judea Pearl's framework draws assumptions as a **directed acyclic graph** (DAG) — the same graph machinery behind [Bayesian networks](ch13.html): nodes are variables, an arrow $X \to Y$ means $X$ is a *direct cause* of $Y$, and "acyclic" forbids a variable causing itself through a loop. The graph is not learned from data — it *encodes your causal assumptions* so they can be inspected and argued with.
...
**Propensity scores.** Matching on many covariates is hard (the [curse of dimensionality](ch07.html)). The **propensity score** $e(x)=P(T{=}1\mid X{=}x)$ — the modeled probability of being treated — collapses all confounders into one number. Rosenbaum and Rubin proved that adjusting for $e(x)$ alone removes the same bias as adjusting for all of $X$. You then match, stratify, or **inverse-propensity weight** (re-weight each unit by $1/e(x)$ for treated, $1/(1-e(x))$ for control) to build a pseudo-population where treatment is independent of confounders.
...
```{=html}
<style>
@keyframes c37-grow { 0%,100%{r:6px;opacity:.6} 50%{r:16px;opacity:1} }
@media (prefers-reduced-motion: reduce){ .c37-rare{animation:none!important} }
</style>
<svg viewBox="0 0 420 130" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="12" role="img" aria-label="A rare treated unit is up-weighted to rebalance treated and control arms">
  <text x="60" y="22" text-anchor="middle" fill="currentColor">control arm</text>
  <circle cx="30" cy="60" r="7" fill="#38bdf8"/><circle cx="55" cy="60" r="7" fill="#38bdf8"/>
  <circle cx="80" cy="60" r="7" fill="#38bdf8"/><circle cx="45" cy="82" r="7" fill="#38bdf8"/>
  <circle cx="70" cy="82" r="7" fill="#38bdf8"/>
  <text x="210" y="22" text-anchor="middle" fill="currentColor">×1/e &#8594; up-weight the rare treated unit</text>
  <text x="350" y="22" text-anchor="middle" fill="currentColor">treated arm</text>
  <circle class="c37-rare" cx="350" cy="70" r="6" fill="#ec4899" style="animation:c37-grow 3s ease-in-out infinite"/>
  <text x="350" y="112" text-anchor="middle" fill="currentColor">e=0.1 &#8594; weight 10</text>
  <line x1="100" y1="70" x2="320" y2="70" stroke="#a855f7" stroke-width="1.5" stroke-dasharray="4 4" marker-end="url(#c37arr)"/>
  <defs><marker id="c37arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 Z" fill="#a855f7"/></marker></defs>
</svg>
```
...
**Also written:** as the interaction coefficient $\beta_3$ in the [regression](ch08.html) $Y = \beta_0 + \beta_1\,\text{Treat} + \beta_2\,\text{Post} + \beta_3\,(\text{Treat}\times\text{Post}) + \varepsilon$ — the "double difference" is exactly that interaction term.
...
<svg viewBox="0 0 440 220" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="12">
  <line x1="50" y1="190" x2="420" y2="190" stroke="currentColor" opacity="0.4"/>
  <line x1="50" y1="20" x2="50" y2="190" stroke="currentColor" opacity="0.4"/>
  <line x1="235" y1="20" x2="235" y2="190" stroke="currentColor" opacity="0.3" stroke-dasharray="4"/>
  <text x="195" y="15" fill="currentColor" opacity="0.7">treatment time</text>
  <line x1="70" y1="150" x2="235" y2="120" stroke="#38bdf8" stroke-width="2"/>
  <line x1="235" y1="120" x2="400" y2="90" stroke="#38bdf8" stroke-width="2"/>
  <text x="405" y="92" fill="#38bdf8">control</text>
  <line x1="70" y1="120" x2="235" y2="90" stroke="#ec4899" stroke-width="2"/>
  <line x1="235" y1="90" x2="400" y2="40" stroke="#ec4899" stroke-width="2"/>
  <text x="405" y="42" fill="#ec4899">treated</text>
  <line x1="235" y1="90" x2="400" y2="60" stroke="#ec4899" stroke-width="2" stroke-dasharray="5"/>
  <text x="280" y="80" fill="#ec4899" opacity="0.85">counterfactual</text>
  <line x1="400" y1="40" x2="400" y2="60" stroke="#f59e0b" stroke-width="3"/>
  <text x="332" y="54" fill="#f59e0b">DiD = effect</text>
</svg>
...
<svg viewBox="0 0 440 200" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="12">
  <line x1="50" y1="170" x2="420" y2="170" stroke="currentColor" opacity="0.4"/>
  <line x1="50" y1="20" x2="50" y2="170" stroke="currentColor" opacity="0.4"/>
  <line x1="235" y1="20" x2="235" y2="180" stroke="currentColor" opacity="0.3" stroke-dasharray="4"/>
  <text x="200" y="195" fill="currentColor" opacity="0.7">cutoff</text>
  <text x="120" y="195" fill="currentColor" opacity="0.7">running variable (e.g. test score)</text>
...
```{=html}
<style>
@keyframes c37-pulse { 0%,100%{transform:scale(1)} 50%{transform:scale(1.18)} }
@media (prefers-reduced-motion: reduce){ .c37-pers{animation:none!important} }
</style>
<svg viewBox="0 0 460 130" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="11" role="img" aria-label="Four customer types ranked by uplift; only persuadables have positive uplift worth targeting">
  <text x="230" y="16" text-anchor="middle" fill="currentColor" font-weight="bold">uplift = P(buy | emailed) − P(buy | left alone)</text>
  <!-- persuadable: positive, glowing -->
  <g class="c37-pers" style="animation:c37-pulse 2.6s ease-in-out infinite;transform-origin:70px 70px">
    <rect x="35" y="45" width="70" height="50" rx="8" fill="#22c55e" opacity="0.35"/>
  </g>
  <text x="70" y="68" text-anchor="middle" fill="currentColor">persuadable</text>
  <text x="70" y="85" text-anchor="middle" fill="#22c55e" font-weight="bold">+ target</text>
  <!-- sure thing -->
  <rect x="125" y="45" width="70" height="50" rx="8" fill="#38bdf8" opacity="0.2"/>
  <text x="160" y="68" text-anchor="middle" fill="currentColor">sure thing</text>
  <text x="160" y="85" text-anchor="middle" fill="#38bdf8">0 skip</text>
  <!-- lost cause -->
  <rect x="215" y="45" width="70" height="50" rx="8" fill="#38bdf8" opacity="0.2"/>
  <text x="250" y="68" text-anchor="middle" fill="currentColor">lost cause</text>
  <text x="250" y="85" text-anchor="middle" fill="#38bdf8">0 skip</text>
  <!-- sleeping dog: negative -->
  <rect x="305" y="45" width="80" height="50" rx="8" fill="#ec4899" opacity="0.3"/>
  <text x="345" y="68" text-anchor="middle" fill="currentColor">sleeping dog</text>
  <text x="345" y="85" text-anchor="middle" fill="#ec4899" font-weight="bold">− avoid</text>
  <text x="425" y="74" fill="currentColor">uplift →</text>
</svg>
```
...
- [**Probability & Statistics**](ch04.html) — distributions, expectation, hypothesis testing, the foundation for ATE estimation and A/B significance.
- [**Probabilistic Graphical Models**](ch13.html) — Bayesian networks and d-separation, the machinery behind DAGs and the backdoor criterion.
- [**Model Evaluation & Tuning**](ch12.html) — experimental design, cross-validation, and the testing logic that A/B tests build on.
- [**Explainable AI & Interpretability**](ch36.html) — counterfactual explanations and recourse, the applied cousins of causal queries.
- [**AI Ethics, Fairness & Safety**](ch38.html) — counterfactual fairness and the harms of acting on spurious correlation.
- [**Frontier & Emerging Directions**](ch39.html) — causal representation learning and invariance for out-of-distribution robustness.

------------------------------------------------------------------------

::: {.callout-note appearance="simple"}
**↪ The thread continues → [Chapter 38 · ⚖️ AI Ethics, Fairness & Safety](ch38.html)**
