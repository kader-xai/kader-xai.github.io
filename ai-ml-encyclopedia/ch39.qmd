...
**In words:** "Out of every candidate in the batch, how confidently does the model pick my true twin as the closest match?" — it's the softmax probability of the positive partner, negated and logged so that *more confident = lower loss*.
**Also written:** as a cross-entropy with a one-hot target on the positive, $\mathcal{L}_i = \text{CE}\big(\text{softmax}_k(\text{sim}(z_i,z_k)/\tau),\, \mathbb{1}[k=j]\big)$, or equivalently $\mathcal{L}_i = -\,\text{sim}(z_i,z_j)/\tau + \log\sum_{k\neq i}\exp(\text{sim}(z_i,z_k)/\tau)$ (numerator term minus a log-sum-exp).
