...
<style>
@keyframes c20-arc-pulse { 0%,100% { opacity:.25; r:6px; } 50% { opacity:1; r:9px; } }
@keyframes c20-arc-flow { 0% { stroke-dashoffset:240; } 100% { stroke-dashoffset:0; } }
.c20-arc-line { stroke-dasharray:6 6; animation: c20-arc-flow 6s linear infinite; }
.c20-arc-dot { animation: c20-arc-pulse 4s ease-in-out infinite; }
@media (prefers-reduced-motion: reduce) { .c20-arc-line,.c20-arc-dot { animation:none; } }
</style>
<svg viewBox="0 0 620 110" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="11">
  <line class="c20-arc-line" x1="40" y1="55" x2="580" y2="55" stroke="#6366f1" stroke-width="2" stroke-opacity="0.7"/>
  <g fill="currentColor" text-anchor="middle">
    <circle class="c20-arc-dot" cx="55"  cy="55" r="6" fill="#38bdf8" style="animation-delay:0s"/>
    <text x="55"  y="80">text</text>
    <circle class="c20-arc-dot" cx="185" cy="55" r="6" fill="#22c55e" style="animation-delay:.8s"/>
    <text x="185" y="80">tokens</text>
    <circle class="c20-arc-dot" cx="315" cy="55" r="6" fill="#f59e0b" style="animation-delay:1.6s"/>
    <text x="315" y="80">counts / embeddings</text>
    <circle class="c20-arc-dot" cx="445" cy="55" r="6" fill="#ec4899" style="animation-delay:2.4s"/>
    <text x="445" y="80">language models</text>
    <circle class="c20-arc-dot" cx="565" cy="55" r="6" fill="#a855f7" style="animation-delay:3.2s"/>
    <text x="565" y="80">tasks</text>
  </g>
  <text x="40" y="28" fill="currentColor" font-size="12" opacity="0.8">The NLP ladder: each rung carries more meaning than the last</text>
</svg>
...
<style>
@keyframes c20-bpe-snap { 0%,12% { transform:translateX(0); opacity:.55; } 25%,100% { transform:translateX(0); opacity:1; } }
@keyframes c20-bpe-glow { 0%,100% { fill-opacity:.35; } 50% { fill-opacity:.7; } }
.c20-bpe-brick { animation: c20-bpe-glow 4s ease-in-out infinite; }
@media (prefers-reduced-motion: reduce) { .c20-bpe-brick { animation:none; } }
</style>
<svg viewBox="0 0 460 130" xmlns="http://www.w3.org/2000/svg" font-family="monospace" font-size="14">
  <text x="10" y="22" fill="currentColor" font-family="sans-serif" font-size="12">"slowest" → reuse known bricks, zero OOV</text>
  <g text-anchor="middle">
    <rect class="c20-bpe-brick" x="40"  y="45" width="40" height="38" rx="6" fill="#38bdf8" style="animation-delay:0s"/>
    <text x="60"  y="70" fill="currentColor">s</text>
    <rect class="c20-bpe-brick" x="95"  y="45" width="70" height="38" rx="6" fill="#22c55e" style="animation-delay:.6s"/>
    <text x="130" y="70" fill="currentColor">low</text>
    <rect class="c20-bpe-brick" x="180" y="45" width="70" height="38" rx="6" fill="#f59e0b" style="animation-delay:1.2s"/>
    <text x="215" y="70" fill="currentColor">est</text>
  </g>
  <text x="40" y="108" fill="currentColor" font-family="sans-serif" font-size="11" opacity="0.75">each brick was learned earlier from "low", "lower", "newest"</text>
</svg>
...
::: {.callout-warning}
BoW/TF-IDF throw away word order. "Dog bites man" and "man bites dog" get identical vectors. When order or negation matters, you need sequence models ([Recurrent & Sequence Models](ch16.html)) or transformers ([Attention & Transformers](ch17.html)).
:::
...
<style>
@keyframes c20-emb-pulse { 0%,100% { r:7px; opacity:1; } 50% { r:11px; opacity:.85; } }
@keyframes c20-emb-near { 0%,100% { fill-opacity:.4; } 50% { fill-opacity:.85; } }
.c20-emb-q { animation: c20-emb-pulse 3.5s ease-in-out infinite; }
.c20-emb-n { animation: c20-emb-near 3.5s ease-in-out infinite; }
@media (prefers-reduced-motion: reduce) { .c20-emb-q,.c20-emb-n { animation:none; } }
</style>
<svg viewBox="0 0 420 200" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="12">
  <text x="10" y="20" fill="currentColor" font-size="13">Meaning space: neighbors of "king"</text>
  <line x1="200" y1="110" x2="150" y2="70"  stroke="#a855f7" stroke-opacity="0.4"/>
  <line x1="200" y1="110" x2="270" y2="65"  stroke="#a855f7" stroke-opacity="0.4"/>
  <line x1="200" y1="110" x2="285" y2="135" stroke="#a855f7" stroke-opacity="0.4"/>
  <circle class="c20-emb-q" cx="200" cy="110" r="7" fill="#a855f7"/>
  <text x="208" y="128" fill="currentColor">king</text>
  <circle class="c20-emb-n" cx="150" cy="70"  r="6" fill="#ec4899" style="animation-delay:.3s"/><text x="100" y="66" fill="currentColor">queen</text>
  <circle class="c20-emb-n" cx="270" cy="65"  r="6" fill="#22c55e" style="animation-delay:.9s"/><text x="278" y="60" fill="currentColor">prince</text>
  <circle class="c20-emb-n" cx="285" cy="135" r="6" fill="#f59e0b" style="animation-delay:1.5s"/><text x="293" y="139" fill="currentColor">throne</text>
  <circle cx="70"  cy="165" r="5" fill="#38bdf8" fill-opacity="0.5"/><text x="20" y="169" fill="currentColor" opacity="0.7">banana</text>
  <text x="20" y="190" fill="currentColor" font-size="11" opacity="0.7">unrelated words sit far away</text>
</svg>
...
In skip-gram, the model wants the dot product of a center word's vector $v_c$ and a true context word's vector $u_o$ to be large; the [softmax](ch14.html) over the vocabulary gives the probability
...
**GloVe** (Global Vectors) reaches a similar place from a different angle: it [factorizes](ch07.html) a global word **co-occurrence matrix**, fitting vectors so that $v_i^\top v_j$ approximates $\log$ of how often words $i$ and $j$ co-occur. word2vec is "predictive/local," GloVe is "count-based/global"; in practice they yield comparably useful vectors.
...
<svg viewBox="0 0 560 230" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="11">
  <text x="10" y="18" fill="currentColor" font-size="13">Skip-gram: one-hot → row-lookup v_c → context logits</text>
  <!-- one-hot input -->
  <text x="50" y="44" fill="currentColor" text-anchor="middle">one-hot (V)</text>
  <g>
    <rect x="30" y="52" width="40" height="18" fill="#6366f1" fill-opacity="0.25" stroke="currentColor" stroke-opacity="0.4"/>
    <rect x="30" y="70" width="40" height="18" fill="#22c55e" fill-opacity="0.8" stroke="currentColor" stroke-opacity="0.4"/>
    <text x="50" y="84" fill="currentColor" text-anchor="middle">1</text>
    <rect x="30" y="88" width="40" height="18" fill="#6366f1" fill-opacity="0.25" stroke="currentColor" stroke-opacity="0.4"/>
    <rect x="30" y="106" width="40" height="18" fill="#6366f1" fill-opacity="0.25" stroke="currentColor" stroke-opacity="0.4"/>
  </g>
  <text x="50" y="142" fill="currentColor" text-anchor="middle" font-size="10" opacity="0.7">word = "king"</text>
  <!-- W matrix -->
  <line x1="75" y1="80" x2="120" y2="80" stroke="currentColor" stroke-opacity="0.5" marker-end="url(#w2v-arr)"/>
  <rect x="125" y="55" width="80" height="70" fill="#38bdf8" fill-opacity="0.3" stroke="currentColor" stroke-opacity="0.4"/>
  <text x="165" y="92" fill="currentColor" text-anchor="middle">W (V×d)</text>
  <rect x="125" y="73" width="80" height="16" fill="#22c55e" fill-opacity="0.55"/>
  <text x="165" y="142" fill="currentColor" text-anchor="middle" font-size="10" opacity="0.7">row lookup</text>
  <!-- v_c -->
  <line x1="210" y1="80" x2="250" y2="80" stroke="currentColor" stroke-opacity="0.5" marker-end="url(#w2v-arr)"/>
  <rect x="255" y="68" width="60" height="24" fill="#a855f7" fill-opacity="0.6" stroke="currentColor" stroke-opacity="0.4"/>
  <text x="285" y="84" fill="currentColor" text-anchor="middle">v_c (d)</text>
  <text x="285" y="142" fill="currentColor" text-anchor="middle" font-size="10" opacity="0.7">center vector</text>
  <!-- W' matrix -->
  <line x1="320" y1="80" x2="360" y2="80" stroke="currentColor" stroke-opacity="0.5" marker-end="url(#w2v-arr)"/>
  <rect x="365" y="55" width="80" height="70" fill="#f59e0b" fill-opacity="0.3" stroke="currentColor" stroke-opacity="0.4"/>
  <text x="405" y="92" fill="currentColor" text-anchor="middle">W' (d×V)</text>
  <text x="405" y="142" fill="currentColor" text-anchor="middle" font-size="10" opacity="0.7">context vectors u_w</text>
  <!-- softmax out -->
  <line x1="450" y1="80" x2="490" y2="80" stroke="currentColor" stroke-opacity="0.5" marker-end="url(#w2v-arr)"/>
  <g>
    <rect x="495" y="52" width="40" height="14" fill="#ec4899" fill-opacity="0.4"/>
    <rect x="495" y="66" width="40" height="14" fill="#ec4899" fill-opacity="0.8"/>
    <rect x="495" y="80" width="40" height="14" fill="#ec4899" fill-opacity="0.3"/>
    <rect x="495" y="94" width="40" height="14" fill="#ec4899" fill-opacity="0.5"/>
  </g>
  <text x="515" y="125" fill="currentColor" text-anchor="middle" font-size="10" opacity="0.7">P(context)</text>
  <text x="40" y="185" fill="currentColor" font-size="11" opacity="0.8">Two tables: W gives each word a center vector, W' a context vector. Keep W.</text>
  <defs><marker id="w2v-arr" markerWidth="8" markerHeight="8" refX="6" refY="3" orient="auto"><path d="M0,0 L6,3 L0,6 z" fill="currentColor"/></marker></defs>
</svg>
...
**In words:** make the real context word's vector align with the center vector (drive $\sigma(u_o^\top v_c)$ toward 1, so its log toward 0), and make each random negative word's vector *anti*-align (drive $\sigma(-u_{w_i}^\top v_c)$ toward 1, i.e. $u_{w_i}^\top v_c$ toward $-\infty$). One "yes" and $k$ "no"s per step.
**Also written:** using $\sigma(-x) = 1 - \sigma(x)$, the equivalent **minimized loss** is $\mathcal{L} = -\log \sigma(u_o^\top v_c) - \sum_{i=1}^{k} \log\big(1 - \sigma(u_{w_i}^\top v_c)\big)$ — a binary cross-entropy with label 1 on the true pair and label 0 on each negative.
...
This is precisely the gap that **contextual** models closed. By reading the whole sentence, [Attention & Transformers](ch17.html) give each *occurrence* of a word its own vector — "bank" near "river" and "bank" near "loan" come out different — and that contextual representation is the foundation the modern [Large Language Models](ch23.html) are built on. word2vec is where the embedding idea was made practical; contextual embeddings are where it was made dynamic. (For the geometry-of-vectors ideas underlying all of this, see [dimensionality reduction](ch07.html); for the same skip-gram trick applied to graphs rather than text, see [node2vec](ch40.html).)
...
The pipeline is the general supervised-learning loop ([AI, ML & the Learning Process](ch05.html)) wearing a text costume: turn each document into a vector (TF-IDF or embeddings), then train any [classifier](ch09.html) (logistic regression, Naive Bayes, an LSTM, or a fine-tuned transformer).
...
The trap in sentiment is **negation and sarcasm**: "not great" flips polarity but a bag-of-words model sees the positive word "great". Word-order-aware models (LSTM, transformer) handle this far better, which is why fine-tuned transformers now dominate sentiment benchmarks. For evaluating these classifiers — accuracy, precision, recall, F1 — see Section 20.11 and [Model Evaluation & Tuning](ch12.html).
...
Translation is one instance of a much broader family: **conditional text generation** ([Generative Models](ch18.html)), where a model produces an output sequence from some input. **Summarization** is the other everyday member — condensing a long document into a short one — and it splits into two styles. **Extractive** summarization *copies* the most important sentences verbatim (safe, never hallucinates, but choppy); **abstractive** summarization *rewrites* the gist in new words with a seq2seq or LLM (fluent, but can invent facts). The same encoder–decoder machinery from translation powers abstractive summarization; only the training data changes (document → summary instead of source → target).
...
<style>
@keyframes c20-temp-top { 0%,100% { height:64px; y:36px; } 50% { height:30px; y:70px; } }
@keyframes c20-temp-mid { 0%,100% { height:24px; y:76px; } 50% { height:26px; y:74px; } }
@keyframes c20-temp-low { 0%,100% { height:8px;  y:92px; } 50% { height:22px; y:78px; } }
@keyframes c20-temp-label { 0%,100% { opacity:1; } 50% { opacity:.25; } }
@keyframes c20-temp-label2 { 0%,100% { opacity:.25; } 50% { opacity:1; } }
.c20-temp-top { animation: c20-temp-top 5s ease-in-out infinite; }
.c20-temp-mid { animation: c20-temp-mid 5s ease-in-out infinite; }
.c20-temp-low { animation: c20-temp-low 5s ease-in-out infinite; }
.c20-temp-lo  { animation: c20-temp-label 5s ease-in-out infinite; }
.c20-temp-hi  { animation: c20-temp-label2 5s ease-in-out infinite; }
@media (prefers-reduced-motion: reduce) { .c20-temp-top,.c20-temp-mid,.c20-temp-low,.c20-temp-lo,.c20-temp-hi { animation:none; } }
</style>
<svg viewBox="0 0 360 130" xmlns="http://www.w3.org/2000/svg" font-family="sans-serif" font-size="11">
  <text x="10" y="18" fill="currentColor" font-size="12">One temperature dial, two moods</text>
  <line x1="40" y1="100" x2="320" y2="100" stroke="currentColor" stroke-opacity="0.4"/>
  <rect class="c20-temp-top" x="60"  width="40" rx="3" fill="#6366f1" fill-opacity="0.8"/>
  <rect class="c20-temp-mid" x="120" width="40" rx="3" fill="#6366f1" fill-opacity="0.6"/>
  <rect class="c20-temp-low" x="180" width="40" rx="3" fill="#6366f1" fill-opacity="0.4"/>
  <text class="c20-temp-lo" x="245" y="50" fill="#22c55e">low T: sharp,</text>
  <text class="c20-temp-lo" x="245" y="64" fill="#22c55e">safe pick</text>
  <text class="c20-temp-hi" x="245" y="88" fill="#f59e0b">high T: flat,</text>
  <text class="c20-temp-hi" x="245" y="102" fill="#f59e0b">creative</text>
</svg>
...
For the LLM techniques behind modern chatbots — instruction tuning, RAG, function calling — see [Large Language Models](ch23.html).
...
::: {.callout-note appearance="simple"}
**↪ The thread continues → [Chapter 21 · 🔊 Speech & Audio Processing](ch21.html)**
