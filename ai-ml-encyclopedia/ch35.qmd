...
$$P(i \to j) \;\propto\; \tau_{ij}^{\alpha}\,\eta_{ij}^{\beta}$$
...
## 35.7 — When Metaheuristics Beat Gradient Methods (and the No-Free-Lunch Caveat)
...
- **Metaheuristics** are gradient-free, general-purpose search strategies for black-box, combinatorial, and multi-modal problems where calculus cannot start.
- **Genetic algorithms** evolve a population via selection, crossover, mutation, and elitism; watch for premature convergence. **Genetic programming** evolves programs/formulas (scored by running the tree on data) and fights code bloat.
- **Evolution strategies** mutate real vectors with self-adapting step sizes via the windowed 1/5 success rule (CMA-ES being the strong modern variant); **neuroevolution** (e.g. NEAT) evolves network weights and even topology.
- **Swarm intelligence**: PSO swirls particles toward personal and global bests in continuous space; ACO lays pheromone (with evaporation + reinforcement) to solve graph/routing problems.
- **Simulated annealing** escapes local optima by probabilistically accepting worse moves under a cooling schedule; **tabu search** uses a forbidden-move memory to keep exploring.
- **Fuzzy logic** reasons with degrees of truth via membership functions and `IF–THEN` rules, giving smooth, expert-driven control without training data (28°C → warm 0.33, hot 0.33 → fan 65%).
