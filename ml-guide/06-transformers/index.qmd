...
To figure out how much word A should attend to word B, you compare A's *query* against B's *key* — and that comparison is just a [dot product](../../ai-ml-encyclopedia/ch01.html), the same "how aligned are these?" measure from the very first chapter. High match, high attention. Those match-scores are softened into weights that sum to one, and the word's output becomes the weighted blend of everyone's *values*. Query finds, key matches, value delivers.
