...
For all its visual magic, a convolutional network is still the network of the first three chapters. The filter's multiply-and-sum is a [dot product](../../ml-simplified.html). The bend after it is still ReLU. It still measures its error with a loss, still rolls downhill by gradient descent, still assigns blame by backpropagation. Convolution didn't replace the neural network — it *reshaped* it, swapping dense everything-to-everything connections for small, shared, sliding ones that fit the grid-like nature of images like a key in a lock.
