...
(function () {
  "use strict";
  const P = "#c2410c", P2 = "#d98e4f", TEAL = "#5f6c5d", GOLD = "#b8860b", ROSE = "#2f4858";
  const reg = {};
  ...
  function colBg() { return isDark() ? "#181511" : "#fffdf9"; }
  ...
  function colTxt() { return isDark() ? "#d9d4ca" : "#221e18"; }
  ...
  function vecDemo(host, p, mode) {
    ...
    function draw() {
      ...
      if (mode === "add") {
        ...
        C.ctx.setLineDash([4, 4]); arrow(C, a[0], a[1], s[0], s[1], "rgba(217,142,79,.7)", 1.5);
        arrow(C, b[0], b[1], s[0], s[1], "rgba(217,142,79,.7)", 1.5); C.ctx.setLineDash([]);
        ...
        out.innerHTML = `a·b = <b>${d.toFixed(2)}</b> → ${perp ? "<b style='color:#5f6c5d'>⟂ orthogonal (90°)</b>" : "not perpendicular — drag to make a·b = 0"}`;
      }
    }
  ...
  register("norm", function (host, p) {
    ...
    function draw() {
      ...
      if (kind === "l1") {
        ...
        C.ctx.strokeStyle = "rgba(217,142,79,.5)"; C.ctx.lineWidth = 1.5;
  ...
  register("cross", function (host, p) {
    ...
    function draw() {
      ...
      C.ctx.fillStyle = "rgba(194,65,12,.15)";
  ...
  register("lintransform", function (host, p) {
    ...
    function draw() {
      ...
      C.ctx.fillStyle = "rgba(194,65,12,.16)";
  ...
  register("gradientfield", function (host, p) {
    ...
    function draw() {
      clear(C);
      for (let r = 0.5; r <= 3; r += 0.5) { C.ctx.strokeStyle = "rgba(194,65,12,.25)"; C.ctx.beginPath(); C.ctx.arc(C.sx(0), C.sy(0), Math.abs(C.sx(r) - C.sx(0)), 0, 7); C.ctx.stroke(); }
      grid(C, 1);
  ...
  register("search", function (host, p) {
    ...
    let best = -9, trials = [];
    const ctrls = controls(host); const out = readout(host, "");
    function draw() { clear(C); grid(C, 1); plotFn(C, f, "rgba(194,65,12,.4)", 1.6); trials.forEach(t => dot(C, t, f(t), t === best ? ROSE : P, t === best ? 7 : 4)); out.innerHTML = `trials: <b>${trials.length}</b> &nbsp; best score: <b>${best > -9 ? f(best).toFixed(2) : "—"}</b>`; }
  ...
  register("scatter", function (host, p) {
    ...
    function draw() {
      clear(C); grid(C, 1);
      for (let i = 0; i < 140; i++) { const z1 = gauss(), z2 = gauss(); const x = z1, y = rho * z1 + Math.sqrt(1 - rho * rho) * z2; dot(C, x, y, "rgba(194,65,12,.55)", 3); }
  ...
  register("hyptest", function (host, p) {
    ...
    function draw() {
      clear(C); grid(C, 1);
      plotFn(C, x => Math.exp(-x * x / 2) / Math.sqrt(2 * Math.PI), "rgba(194,65,12,.7)", 2.2);       // H0
  ...
  register("pvalue", function (host, p) {
    ...
    function draw() {
      ...
      const pp = tail(z);
      out.innerHTML = `z = <b>${z.toFixed(2)}</b> → p ≈ <b>${pp.toFixed(3)}</b> (rose tail area). ${pp < 0.05 ? "<b style='color:#5f6c5d'>significant (p<0.05)</b>" : "not significant"}`;
    }
  ...
  register("sampling", function (host, p) {
    ...
    function draw() {
      clear(C); grid(C, 1); plotFn(C, x => Math.exp(-x * x / 2) / Math.sqrt(2 * Math.PI), "rgba(194,65,12,.5)", 2);
